...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase11_planner"
...
SUMMARY_MD = OUTPUT_DIR / "phase11_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
ENERGY_ERROR_PNG = OUTPUT_DIR / "energy_error_summary.png"
...
PLANNER_MODES = ["energy_correction", "blend_window", "soft_window"]
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase11_energy_planner", True),
    ]
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase11_energy_planner"]
...
def save_energy_error_summary(rows: Sequence[Dict[str, object]]) -> None:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase11_energy_planner"]
    labels = [f"{row['r0_over_target']:g}/{row['initial_velocity_angle_deg']:g}/{row['thrust_scale']:g}" for row in planner_rows]
    initial = [float(row["initial_energy_error_ratio"]) for row in planner_rows]
    final = [float(row["final_energy_error_ratio"]) for row in planner_rows]
    x = np.arange(len(labels))
    fig, ax = plt.subplots(figsize=(11.0, 4.8))
    ax.plot(x, initial, label="initial", color="#D95F02", linewidth=1.1)
    ax.plot(x, final, label="final", color="#386CB0", linewidth=1.1)
    ax.set_xticks(x[::4])
    ax.set_xticklabels(labels[::4], rotation=35, ha="right", fontsize=7)
    ax.set_ylabel("energy error ratio")
    ax.set_title("Phase 11 planner energy-error change by regime")
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(ENERGY_ERROR_PNG, dpi=220)
    plt.close(fig)


def save_capture_improvement(stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase11"]
    baseline = stats["baseline_soft_linear_3e4"]
    planner = stats["phase11_energy_planner"]
    captures = [baseline["capture"], planner["capture"]]
    successes = [baseline["success"], planner["success"]]
    x = np.arange(len(labels))
    ...
    ax.set_title("Phase 11 energy planner vs baseline")
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend()
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    delta_energy = np.asarray(traj["delta_energy"], dtype=np.float64)
    ...
    axes[2].plot(time_days, delta_energy / abs(target_energy), color="#D95F02", linewidth=1.1)
    axes[2].axhline(0.0, color="#333333", linestyle="--", linewidth=1.0)
    axes[2].set_ylabel("delta_E / |target|")
    axes[2].set_xlabel("time [days]")
    axes[2].grid(True, alpha=0.25)

    fig.suptitle(
        f"Phase 11 energy planner r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase11_energy_planner", True)
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    ...
    lines = [
        ...
        f"| `phase11_energy_planner` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        "## Answers",
        "",
        f"1. Does energy-based planning improve capture? It `{honest_result}` CAPTURE on this reduced grid: CAPTURE changes by `{capture_improvement}`.",
        f"2. Does it reduce `no_capture_access`? `{'yes' if no_capture_planner < no_capture_baseline else 'no'}`. No-CAPTURE cases change from `{no_capture_baseline}` to `{no_capture_planner}`.",
        f"3. Does it avoid overspeed? `{'yes' if overspeed_planner == 0 else 'no'}`. The planner produced `{overspeed_planner}` overspeed terminations.",
        f"4. Is trajectory shaping more effective than local control? On this small grid, `{'yes' if helps else 'not yet'}`. Energy-guided planning is more interpretable than local classification, but this lightweight version still needs better angular-momentum and radial-crossing planning before it can beat the Phase 7.6 single-controller baseline.",
        "",
        "## Artifacts",
        "",
        "- `comparison.csv`",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase11 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
