...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "residual_explicit_magnitude_only"
MODEL_DIR = PROJECT_ROOT / "models"
MODEL_PATH = MODEL_DIR / "residual_explicit_magnitude_only_policy.pth"
...
NOTE_PATH = PROJECT_ROOT / "analysis" / "residual_explicit_magnitude_only_result.md"
...
ALPHA = 0.05
RAW_RESIDUAL_CLIP = 0.10
MAX_MAGNITUDE_DELTA = 0.005
MIN_ACTION_NORM = 1.0e-9
RESIDUAL_NORM_WEIGHT = 0.01
TAIL_VR_WEIGHT = 0.1
TUNE_LR = 0.02
UPDATE_DIRECTIONS = [1.0, -1.0]
STRICT_CFG = {
    "tol_r": 1.0e-3,
    "tol_v": 1.0e-3,
    "tol_ang": 0.02,
...
class MagnitudeResidualPolicy(nn.Module):
...
def build_magnitude_action(explicit_action: np.ndarray, residual_scalar: float) -> tuple[np.ndarray, Dict[str, float | bool]]:
    explicit_action = np.asarray(explicit_action, dtype=np.float64)
    explicit_magnitude = float(np.linalg.norm(explicit_action))
    if explicit_magnitude < MIN_ACTION_NORM:
        return explicit_action.copy(), {
            "fallback_used": True,
            "explicit_magnitude": explicit_magnitude,
            "new_magnitude": explicit_magnitude,
            "applied_magnitude_delta": 0.0,
            "residual_scalar": 0.0,
        }

    direction = explicit_action / explicit_magnitude
    clipped_residual = float(np.clip(residual_scalar, -RAW_RESIDUAL_CLIP, RAW_RESIDUAL_CLIP))
    requested_magnitude = explicit_magnitude + ALPHA * clipped_residual
    lower = max(0.0, explicit_magnitude - MAX_MAGNITUDE_DELTA)
    upper = explicit_magnitude + MAX_MAGNITUDE_DELTA
    max_component_safe_magnitude = 1.0 / max(float(np.max(np.abs(direction))), 1.0e-12)
    new_magnitude = float(np.clip(requested_magnitude, lower, min(upper, max_component_safe_magnitude)))
    final_action = direction * new_magnitude
    return final_action.astype(np.float64), {
        "fallback_used": False,
        "explicit_magnitude": explicit_magnitude,
        "new_magnitude": new_magnitude,
        "applied_magnitude_delta": new_magnitude - explicit_magnitude,
        "residual_scalar": clipped_residual,
    }
...
def rollout_explicit() -> tuple[Dict[str, float | int | bool | None], Dict[str, List[float]]]:
    ...
    while not (terminated or truncated):
        ...
        final_action, action_info = build_magnitude_action(explicit_action, 0.0)
        ...
        trace["explicit_magnitude"].append(float(action_info["explicit_magnitude"]))
        trace["final_magnitude"].append(float(action_info["new_magnitude"]))
        trace["residual_scalar"].append(0.0)
        trace["applied_magnitude_delta"].append(0.0)
...
def rollout_hybrid(model: MagnitudeResidualPolicy) -> tuple[Dict[str, float | int | bool | None], Dict[str, List[float]]]:
    env = make_env()
    controller = make_controller(env)
    set_default_start(env)
    obs = np.asarray(env._get_obs(), dtype=np.float32)
    ...
    while not (terminated or truncated):
        ...
        with torch.no_grad():
            raw_residual = float(model(x_tensor).squeeze().detach().cpu().item())
        final_action, action_info = build_magnitude_action(explicit_action, raw_residual)

        next_obs, _, terminated, truncated, step_info = env.step(final_action)
        radius = float(np.linalg.norm(env.pos))
        r_hat = env.pos / (radius + 1e-12)
        v_r = float(np.dot(env.vel, r_hat))
        residual_scalar = float(action_info["residual_scalar"])
        ...
        trace["explicit_magnitude"].append(float(action_info["explicit_magnitude"]))
        trace["final_magnitude"].append(float(action_info["new_magnitude"]))
        trace["residual_scalar"].append(residual_scalar)
        trace["applied_magnitude_delta"].append(float(action_info["applied_magnitude_delta"]))
        radius_values.append(radius)
        vr_values.append(v_r)
        residual_values.append(residual_scalar)
        success = success or bool(step_info.get("success", False))
...
def make_model() -> MagnitudeResidualPolicy:
    model = MagnitudeResidualPolicy(input_dim=HISTORY_LEN * 5).to(DEVICE)
    model.eval()
    return model


def set_output_bias(model: MagnitudeResidualPolicy, bias: float) -> None:
    with torch.no_grad():
        model.net[-1].bias.copy_(torch.tensor([bias], dtype=torch.float32, device=DEVICE))


def get_output_bias(model: MagnitudeResidualPolicy) -> float:
    return float(model.net[-1].bias.detach().cpu().numpy().reshape(-1)[0])


def tune_model(explicit_metrics: Dict[str, object]) -> tuple[MagnitudeResidualPolicy, Dict[str, object], List[Dict[str, object]]]:
    model = make_model()
    ...
    for step_idx, direction in enumerate(UPDATE_DIRECTIONS, start=1):
        candidate_bias = float(np.clip(current_bias + TUNE_LR * direction, -RAW_RESIDUAL_CLIP, RAW_RESIDUAL_CLIP))
    ...
    best_summary = {
        ...
        "best_bias": get_output_bias(model),
...
def save_action_compare_plot(path: Path, trace: Dict[str, List[float]]) -> None:
    ...
    plt.plot(trace["step"], trace["explicit_magnitude"], label="explicit magnitude", linewidth=1.1)
    plt.plot(trace["step"], trace["final_magnitude"], label="hybrid magnitude", linewidth=1.1, linestyle="--")
    plt.plot(trace["step"], trace["applied_magnitude_delta"], label="applied magnitude delta", linewidth=0.9)
    plt.plot(trace["step"], np.asarray(trace["residual_scalar"]) * ALPHA, label="alpha residual scalar", linewidth=0.9, alpha=0.8)
    plt.xlabel("step")
    plt.ylabel("action magnitude")
    plt.title("Magnitude-only residual | action comparison")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(path, dpi=180)
    plt.close()
...
def write_note(summary: Dict[str, object]) -> None:
    ...
    accepted_nonzero = [entry for entry in nonzero_attempts if bool(entry["accepted"]) and abs(float(entry["candidate_bias"])) > 0.0]

    if len(harmful_attempts) == len(nonzero_attempts) and nonzero_attempts:
        implication = "Every nonzero magnitude-only residual attempt harmed success, so the accepted checkpoint remained at zero residual."
    elif accepted_nonzero:
        implication = "A nonzero magnitude-only residual was accepted only because it preserved explicit success and improved the rollout objective under a tight magnitude bound."
    else:
        implication = "Magnitude-only residual control preserved the explicit action direction, but this run did not justify moving away from the zero-residual checkpoint."
    ...
    lines = [
        ...
        "## Main Answer",
        "",
        f"- Does magnitude-only residual preserve explicit-controller success? `{'Yes' if preserves_success else 'No'}`",
        ...
        f"- Did every nonzero magnitude-only residual harm success? `{'Yes' if len(harmful_attempts) == len(nonzero_attempts) and nonzero_attempts else 'No'}`",
        f"- What does this imply about constrained hybrid control? {implication}",
        "",
        "## Metrics",
        "",
        f"- alpha `{summary['alpha']}`",
        f"- raw_residual_clip `{summary['raw_residual_clip']}`",
        f"- max_magnitude_delta `{summary['max_magnitude_delta']}`",
    ...
    for entry in summary["optimization_history"]:
        ...
        lines.append(
            ...
            f"candidate_bias `{float(entry['candidate_bias']):.3e}` objective `{metrics['objective']:.6e}` "
            f"success `{metrics['success']}` final_radius_error `{metrics['final_radius_error']:.3e}` "
            f"tail_mean_abs_vr `{metrics['tail_mean_abs_vr']:.3e}`"
        )
...
def main() -> None:
    ...
    save_timeseries_plot(
        RADIUS_PLOT_PATH,
        {"explicit": explicit_trace, "magnitude_only": tuned_trace},
        "radius",
        "Magnitude-only residual | radius vs time",
        "radius [m]",
    ...
    save_timeseries_plot(
        ...
        {"explicit": explicit_trace, "magnitude_only": tuned_trace},
        "v_r",
        "Magnitude-only residual | v_r vs time",
        "v_r [m/s]",
        target_value=0.0,
    )
    ...
    summary = {
        ...
        "best_bias": float(best_summary["best_bias"]),
