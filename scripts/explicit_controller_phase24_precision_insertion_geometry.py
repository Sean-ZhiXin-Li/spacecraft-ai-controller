...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase24_precision_insertion_geometry"
...
RESULTS_CSV = OUTPUT_DIR / "phase24_results.csv"
...
TIMING_DISTRIBUTION_PNG = OUTPUT_DIR / "insertion_timing_distribution.png"
MODE_COMPARISON_PNG = OUTPUT_DIR / "insertion_mode_comparison.png"
BASIN_SCORES_PNG = OUTPUT_DIR / "recoverable_basin_scores.png"
POST_BURN_GEOMETRY_PNG = OUTPUT_DIR / "post_burn_geometry_analysis.png"
...
PRECISION_POST_BURN_HORIZON = 120
PRECISION_MAX_WAIT_STEPS = 96
...
def normalize_reference_row(row: Dict[str, object]) -> Dict[str, object]:
    ...
    out.setdefault("chosen_burn_timing", "")
    out.setdefault("chosen_burn_angle_deg", "")
    out.setdefault("chosen_burn_magnitude", "")
    out.setdefault("chosen_burn_duration", "")
    out.setdefault("chosen_insertion_mode", "")
    out.setdefault("pre_burn_state_json", "{}")
    out.setdefault("post_burn_state_json", "{}")
    out.setdefault("basin_score", "")
    out.setdefault("handoff_success", bool_from_csv(out.get("success")))
    return out
...
def rollout_phase24_case(
    ...
    insertion_plan: PrecisionInsertionPlan | None = None
    insertion_wait_remaining = 0
    insertion_burn_remaining = 0
    chosen_burn_timing: int | None = None
    chosen_burn_angle_deg: float | None = None
    chosen_burn_magnitude: float | None = None
    chosen_burn_duration: int | None = None
    chosen_insertion_mode = ""
    ...
    def burn_b_action(state_x: float, state_y: float, state_vx: float, state_vy: float, diag) -> tuple[float, float, str]:
        nonlocal burn_b_solver_evals, insertion_plan, insertion_wait_remaining, insertion_burn_remaining
        nonlocal chosen_burn_timing, chosen_burn_angle_deg, chosen_burn_magnitude, chosen_burn_duration, chosen_insertion_mode
        nonlocal pre_burn_state, post_burn_state

        def state_payload(test_diag) -> Dict[str, float]:
            return {
                "r_error_ratio": float(test_diag.r_error_ratio),
                "vr_ratio": float(test_diag.radial_velocity / (v_circ + 1.0e-12)),
                "vt_error_ratio": float(test_diag.vt_error_ratio),
                "energy_error_ratio": float(test_diag.energy_error_ratio),
                "angular_momentum_error_ratio": float(test_diag.angular_momentum_error_ratio),
            }

        def basin_score(test_diag, speed_ratio: float, crossed: bool) -> float:
            test_vr_ratio = test_diag.radial_velocity / (v_circ + 1.0e-12)
            score = (
                1.00 * abs(test_diag.r_error_ratio)
                + 1.35 * abs(test_vr_ratio)
                + 1.10 * abs(test_diag.vt_error_ratio)
                + 0.22 * abs(test_diag.energy_error_ratio)
                + 0.22 * abs(test_diag.angular_momentum_error_ratio)
            )
            if crossed:
                score -= 0.08
            ...
            if speed_ratio > SAFE_SPEED_RATIO:
                score += 10.0 * (speed_ratio - SAFE_SPEED_RATIO)
            return score

        def solve_plan() -> PrecisionInsertionPlan:
            nonlocal burn_b_solver_evals, pre_burn_state, post_burn_state

            predicted = estimate_crossing_steps(diag.radius - target_radius, diag.radial_velocity)
            center_wait = int(clamp(float(predicted if predicted is not None else 0), 0.0, float(PRECISION_MAX_WAIT_STEPS)))
            timing_offsets = [-36, -18, 0, 18, 36]
            modes = ["radial_priority", "tangential_priority", "balanced_circularization", "timing_first", "minimum_energy_correction"]
            magnitudes = [0.18, 0.30, 0.42]
            durations = [6, 12, 20]
            best_plan = PrecisionInsertionPlan("coast", 0, 0.0, 0.0, 0.0, 0, float("inf"), 0.0, 0.0)
            best_post_state: Dict[str, float] = {}

            for offset in timing_offsets:
                wait_steps = int(clamp(float(center_wait + offset), 0.0, float(PRECISION_MAX_WAIT_STEPS)))
                wait_x, wait_y, wait_vx, wait_vy = state_x, state_y, state_vx, state_vy
                for _ in range(wait_steps):
                    wait_x, wait_y, wait_vx, wait_vy = env_step(wait_x, wait_y, wait_vx, wait_vy, 0.0, 0.0)
                wait_diag = orbital_diagnostics(wait_x, wait_y, wait_vx, wait_vy, target_radius)
                wait_vr_ratio = wait_diag.radial_velocity / (v_circ + 1.0e-12)

                for mode in modes:
                    if mode == "radial_priority":
                        radial_unit = clamp(-4.5 * wait_vr_ratio - 0.30 * wait_diag.r_error_ratio, -1.0, 1.0)
                        tangential_unit = clamp(-0.8 * wait_diag.vt_error_ratio, -0.6, 0.6)
                    elif mode == "tangential_priority":
                        radial_unit = clamp(-1.4 * wait_vr_ratio, -0.6, 0.6)
                        tangential_unit = clamp(-3.2 * wait_diag.vt_error_ratio, -1.0, 1.0)
                    elif mode == "balanced_circularization":
                        radial_unit = clamp(-2.8 * wait_vr_ratio - 0.25 * wait_diag.r_error_ratio, -0.9, 0.9)
                        tangential_unit = clamp(-2.4 * wait_diag.vt_error_ratio, -0.9, 0.9)
                    elif mode == "timing_first":
                        radial_unit = clamp(-2.0 * wait_vr_ratio, -0.7, 0.7)
                        tangential_unit = clamp(-1.4 * wait_diag.vt_error_ratio, -0.7, 0.7)
                    else:
                        radial_unit = clamp(-1.2 * wait_vr_ratio, -0.5, 0.5)
                        tangential_unit = -1.0 if wait_diag.energy_error_ratio > 0.0 else 1.0

                    norm = math.sqrt(radial_unit * radial_unit + tangential_unit * tangential_unit)
                    if norm > 1.0e-12:
                        radial_unit /= norm
                        tangential_unit /= norm

                    for magnitude in magnitudes:
                        for duration in durations:
                            radial_cmd = radial_unit * magnitude
                            tangential_cmd = tangential_unit * magnitude
                            ...
                            sim_x, sim_y, sim_vx, sim_vy = wait_x, wait_y, wait_vx, wait_vy
                            prev_sim_r_error = wait_diag.radius - target_radius
                            crossed = False
                            best_score = float("inf")
                            best_state: Dict[str, float] = {}
                            for horizon_step in range(duration + PRECISION_POST_BURN_HORIZON):
                                burn_now = horizon_step < duration
                                sim_x, sim_y, sim_vx, sim_vy = env_step(sim_x, sim_y, sim_vx, sim_vy, action_x if burn_now else 0.0, action_y if burn_now else 0.0)
                                sim_diag = orbital_diagnostics(sim_x, sim_y, sim_vx, sim_vy, target_radius)
                                sim_r_error = sim_diag.radius - target_radius
                                crossed = crossed or (prev_sim_r_error > 0.0 and sim_r_error <= 0.0) or (prev_sim_r_error < 0.0 and sim_r_error >= 0.0)
                                prev_sim_r_error = sim_r_error
                                sim_speed_ratio = sim_diag.speed / (v_circ + 1.0e-12)
                                if sim_diag.radius < 0.40 * target_radius or sim_diag.radius > 2.35 * target_radius:
                                    best_score = float("inf")
                                    break
                                score = basin_score(sim_diag, sim_speed_ratio, crossed)
                                if score < best_score:
                                    best_score = score
                                    best_state = state_payload(sim_diag)
                            burn_b_solver_evals += 1
                            if best_score < best_plan.score:
                                best_plan = PrecisionInsertionPlan(mode, wait_steps, radial_cmd, tangential_cmd, magnitude, duration, best_score, action_x, action_y)
                                best_post_state = best_state

            pre_burn_state = state_payload(diag)
            post_burn_state = best_post_state
            burn_b_solver_scores.append(best_plan.score)
            return best_plan

        if insertion_plan is None:
            insertion_plan = solve_plan()
            insertion_wait_remaining = insertion_plan.wait_steps
            insertion_burn_remaining = insertion_plan.duration
            chosen_burn_timing = insertion_plan.wait_steps
            chosen_burn_magnitude = insertion_plan.magnitude
            chosen_burn_duration = insertion_plan.duration
            chosen_insertion_mode = insertion_plan.mode
            chosen_burn_angle_deg = math.degrees(math.atan2(insertion_plan.radial_cmd, insertion_plan.tangential_cmd))

        if insertion_wait_remaining > 0:
            insertion_wait_remaining -= 1
            return 0.0, 0.0, "burn_b_timing_wait"
        if insertion_burn_remaining > 0:
            insertion_burn_remaining -= 1
            return insertion_plan.action_x, insertion_plan.action_y, "burn_b"
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if phase == "DESCENT":
            if speed_ratio > SAFE_SPEED_RATIO or diag.radius < 0.42 * target_radius or diag.radius > 2.25 * target_radius:
                ...
                action_x, action_y, active_stage = burn_b_action(x, y, vx, vy, diag)
                if active_stage == "burn_b":
                    burn_b_steps += 1
                ...
                if active_stage == "phase76_handoff" or burn_b_steps >= BURN_B_MAX_STEPS or abs(diag.r_error_ratio) <= HANDOFF_R_RATIO:
                    transfer_stage = "phase76_handoff"
                    stage_step = 0
    ...
    row: Dict[str, object] = {
        "controller_name": "phase24_precision_insertion_geometry",
        "r0_over_target": r0,
        ...
        "chosen_burn_timing": chosen_burn_timing if chosen_burn_timing is not None else "",
        "chosen_burn_angle_deg": chosen_burn_angle_deg if chosen_burn_angle_deg is not None else "",
        "chosen_burn_magnitude": chosen_burn_magnitude if chosen_burn_magnitude is not None else "",
        "chosen_burn_duration": chosen_burn_duration if chosen_burn_duration is not None else "",
        "chosen_insertion_mode": chosen_insertion_mode,
        "pre_burn_state_json": json.dumps(pre_burn_state, sort_keys=True),
        "post_burn_state_json": json.dumps(post_burn_state, sort_keys=True),
        "basin_score": float(np.min(burn_b_solver_scores)) if burn_b_solver_scores else "",
...
def run_phase24_rows() -> List[Dict[str, object]]:
    controller = ControllerSpec("phase24_precision_insertion_geometry")
    rows: List[Dict[str, object]] = []
    cases = list(iter_cases([controller]))
    for idx, (_controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        row = rollout_phase24_case(r0, angle, thrust, target_scale)
        rows.append(row)
        print(
            f"phase24 {idx}/{len(cases)} r0={r0:g} angle={angle:g} thrust={thrust:g} "
...
def aggregate(rows: Sequence[Dict[str, object]]) -> Dict[str, Dict[str, object]]:
    ...
    for name in sorted({str(row["controller_name"]) for row in rows}):
        ...
        stats[name] = {
            ...
            "mean_basin_score": float(np.mean(numeric_values(subset, "basin_score"))) if numeric_values(subset, "basin_score") else float("nan"),
            "mean_chosen_timing": float(np.mean(numeric_values(subset, "chosen_burn_timing"))) if numeric_values(subset, "chosen_burn_timing") else float("nan"),
        }
    return stats


def phase24_stage_counts(rows: Sequence[Dict[str, object]]) -> Counter[str]:
    counts: Counter[str] = Counter()
    for row in rows:
        if str(row.get("controller_name")) != "phase24_precision_insertion_geometry":
            continue
...
def save_stage_usage_plot(rows: Sequence[Dict[str, object]]) -> None:
    counts = phase24_stage_counts(rows)
    ...
    ax.set_title("Phase 24 transfer stage usage")
...
def save_insertion_window_plot(stats: Dict[str, Dict[str, object]]) -> None:
    phase24 = stats["phase24_precision_insertion_geometry"]
    labels = ["attempts", "windows", "successful windows", "handoffs", "CAPTURE", "success"]
    values = [
        phase24["insertion_attempts"],
        phase24["insertion_windows"],
        phase24["successful_insertion_windows"],
        phase24["handoff_entered"],
        phase24["capture"],
        phase24["success"],
    ]
    fig, ax = plt.subplots(figsize=(8.8, 5.0))
    ax.bar(labels, values, color=["#4C78A8", "#72B7B2", "#54A24B", "#F58518", "#E45756", "#B279A2"])
    ax.set_title("Phase 24 insertion-window outcomes")
    ax.set_ylabel("count")
...
def phase24_rows_only(rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    return [row for row in rows if str(row.get("controller_name")) == "phase24_precision_insertion_geometry"]


def save_precision_insertion_plots(rows: Sequence[Dict[str, object]]) -> None:
    phase_rows = phase24_rows_only(rows)
    timing_values = numeric_values(phase_rows, "chosen_burn_timing")
    basin_scores = numeric_values(phase_rows, "basin_score")
    mode_counts: Counter[str] = Counter(str(row.get("chosen_insertion_mode") or "none") for row in phase_rows if row.get("chosen_insertion_mode"))

    fig, ax = plt.subplots(figsize=(8.6, 4.8))
    if timing_values:
        ax.hist(timing_values, bins=range(0, int(max(timing_values)) + 8, 6), color="#4C78A8", edgecolor="#FFFFFF")
    ax.set_title("Phase 24 chosen insertion timing")
    ax.set_xlabel("wait before Burn B [steps]")
    ax.set_ylabel("cases")
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(TIMING_DISTRIBUTION_PNG, dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8.8, 4.8))
    if mode_counts:
        labels = list(mode_counts.keys())
        ax.bar(labels, [mode_counts[label] for label in labels], color="#54A24B")
        ax.tick_params(axis="x", rotation=20)
    ax.set_title("Phase 24 insertion mode win rate")
    ax.set_ylabel("chosen cases")
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(MODE_COMPARISON_PNG, dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8.8, 4.8))
    if basin_scores:
        ax.plot(sorted(basin_scores), marker="o", linewidth=1.2, color="#F58518")
    ax.set_title("Phase 24 recoverable basin scores")
    ax.set_xlabel("case rank")
    ax.set_ylabel("best basin score")
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(BASIN_SCORES_PNG, dpi=220)
    plt.close(fig)
    ...
    for row in phase_rows:
        payload = str(row.get("post_burn_state_json") or "{}")
        try:
            state = json.loads(payload)
        except json.JSONDecodeError:
            state = {}
        if state:
            post_r.append(float(state.get("r_error_ratio", 0.0)))
            post_vr.append(float(state.get("vr_ratio", 0.0)))
            post_vt.append(float(state.get("vt_error_ratio", 0.0)))
    fig, ax = plt.subplots(figsize=(7.2, 6.0))
    if post_r:
        scatter = ax.scatter(post_vr, post_vt, c=np.abs(post_r), cmap="viridis", s=42)
        fig.colorbar(scatter, ax=ax, label="|r error ratio|")
    ax.axhline(0.0, color="#333333", linestyle="--", linewidth=0.8)
    ax.axvline(0.0, color="#333333", linestyle="--", linewidth=0.8)
    ax.set_title("Phase 24 post-burn geometry")
    ax.set_xlabel("post-burn v_r / v_circ")
    ax.set_ylabel("post-burn v_t error / v_circ")
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(POST_BURN_GEOMETRY_PNG, dpi=220)
    plt.close(fig)
...
def save_diagnostic_plots() -> None:
    ...
    for old_plot in TRAJECTORY_DIR.glob("phase24_representative_*.png"):
        old_plot.unlink()
    trace_rows = [rollout_phase24_case(r0, angle, thrust, TARGET_RADIUS_SCALE, record_trajectory=True) for r0, angle, thrust, _label in representative_cases()]
    ...
    for idx, row in enumerate(trace_rows, start=1):
        ...
        fig.suptitle(f"Phase 24 representative {outcome} transfer {idx}: {label}")
        fig.tight_layout()
        fig.savefig(TRAJECTORY_DIR / f"phase24_representative_{outcome}_{idx:02d}.png", dpi=220)
        plt.close(fig)

    for ax, title, ylabel in [
        (ax_e, "Phase 24 energy error vs time", "|E - E_target| / |E_target|"),
        (ax_h, "Phase 24 angular momentum error vs time", "|h - h_target| / |h_target|"),
    ]:
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, object]]) -> None:
    ...
    phase24 = stats["phase24_precision_insertion_geometry"]
    crossing_improved = int(phase24["crossing_cases"]) > int(baseline["crossing_cases"])
    recoverable_improved = int(phase24["recoverable_crossings"]) > int(phase23["recoverable_crossings"])
    capture_improved = int(phase24["capture"]) > int(baseline["capture"])
    success_improved = int(phase24["success"]) > int(baseline["success"])
    conversion_improved = float(phase24["insertion_window_conversion_rate"]) > float(phase23["insertion_window_conversion_rate"])
    bottleneck = "Phase 7.6 basin too narrow or true physical limitation"
    if int(phase24["recoverable_crossings"]) > 0 and not capture_improved:
        bottleneck = "CAPTURE architecture"
    if int(phase24["insertion_windows"]) == 0 and not crossing_improved:
        bottleneck = "transfer timing before Burn B"
    mode_counts = phase24_stage_counts(rows)
    insertion_modes = Counter(str(row.get("chosen_insertion_mode") or "none") for row in phase24_rows_only(rows) if row.get("chosen_insertion_mode"))
    lines = [
        ...
        f"| Phase 24 `phase24_precision_insertion_geometry` | {phase24['recoverable_crossings']} | {phase24['capture']} | {phase24['success']} | {phase24['insertion_windows']} | {phase24['insertion_window_conversion_rate']:.3f} | {phase24['overspeed']} |",
        "",
        "## Phase 24 Precision Insertion Diagnostics",
        "",
        f"- Burn A steps: `{phase24['burn_a_steps']}`.",
        f"- Burn B steps: `{phase24['burn_b_steps']}`.",
        f"- Precision insertion evaluations: `{phase24['burn_b_solver_evals']}`.",
        f"- Insertion attempts: `{phase24['insertion_attempts']}`.",
        f"- Insertion windows: `{phase24['insertion_windows']}`.",
        f"- Successful insertion windows: `{phase24['successful_insertion_windows']}`.",
        f"- Handoff-entered cases: `{phase24['handoff_entered']}`.",
        f"- Insertion window conversion rate: `{phase24['insertion_window_conversion_rate']:.3f}`.",
        f"- Mean chosen burn timing: `{phase24['mean_chosen_timing']:.2f}` steps.",
        f"- Mean basin score: `{phase24['mean_basin_score']:.4f}`.",
        f"- Insertion mode win counts: `{json.dumps(dict(insertion_modes), sort_keys=True)}`.",
        f"- Stage usage: `{json.dumps(dict(mode_counts), sort_keys=True)}`.",
        f"- Mean final energy error: `{phase24['mean_energy_error']:.4f}`.",
        f"- Mean final angular momentum error: `{phase24['mean_angular_momentum_error']:.4f}`.",
        "",
        "## Research Answers",
        "",
        f"1. Is Burn B timing the dominant missing variable? `{'yes' if conversion_improved else 'no'}` by conversion-rate criterion.",
        f"2. Can insertion geometry outperform brute-force local Burn B? `{'yes' if recoverable_improved or conversion_improved else 'no'}`.",
        f"3. Which insertion mode is strongest? `{max(insertion_modes.items(), key=lambda item: item[1])[0] if insertion_modes else 'none'}`.",
        f"4. Can Phase 24 produce recoverable crossings? `{'yes' if int(phase24['recoverable_crossings']) > 0 else 'no'}`.",
        f"5. If not, current bottleneck: `{bottleneck}`.",
        "",
        "## Success Criteria",
        "",
        f"- Minimum, insertion conversion > Phase 23: `{'met' if conversion_improved else 'not met'}`.",
        f"- Moderate, recoverable crossings > 0: `{'met' if int(phase24['recoverable_crossings']) > 0 else 'not met'}`.",
        ...
        f"- Research success, partial recoverable insertion: `{'met' if int(phase24['recoverable_crossings']) > 0 or conversion_improved else 'not met'}`.",
        "",
        "## Honesty Note",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 24 precision insertion geometry benchmark.")
    ...
    phase24_rows = run_phase24_rows()
    rows = reference_rows + phase24_rows
    write_results(rows)
    stats = aggregate(rows)
    if not args.skip_plots:
        save_stage_usage_plot(rows)
        save_insertion_window_plot(stats)
        save_precision_insertion_plots(rows)
        save_diagnostic_plots()
    write_summary(rows, stats)
    print(f"Saved Phase 24 precision insertion geometry outputs to {OUTPUT_DIR}")
