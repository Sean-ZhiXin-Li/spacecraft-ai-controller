...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase37a_radial_commit_timing"
...
RESULTS_CSV = OUTPUT_DIR / "phase37a_results.csv"
SUMMARY_MD = OUTPUT_DIR / "phase37a_summary.md"
COMPARISON_PNG = OUTPUT_DIR / "phase37a_comparison.png"
...
class Phase37AVariant:
    commit_timing: str
    radial_magnitude_label: str
    radial_magnitude: float

    @property
    def name(self) -> str:
        return f"{self.commit_timing}_{self.radial_magnitude_label}"


VARIANTS = [
    Phase37AVariant("early_commit", "low", RADIAL_MAGNITUDES["low"]),
    Phase37AVariant("early_commit", "medium", RADIAL_MAGNITUDES["medium"]),
    Phase37AVariant("mid_commit", "low", RADIAL_MAGNITUDES["low"]),
    Phase37AVariant("mid_commit", "medium", RADIAL_MAGNITUDES["medium"]),
    Phase37AVariant("delayed_commit", "low", RADIAL_MAGNITUDES["low"]),
    Phase37AVariant("delayed_commit", "medium", RADIAL_MAGNITUDES["medium"]),
]
...
FIELDNAMES = [
    ...
    "variant_name",
    "commit_timing",
    "radial_magnitude_label",
    "radial_magnitude",
    "r0_over_target",
    "initial_velocity_angle_deg",
    "thrust_scale",
    "target_radius_scale",
    "baseline_non_crossing_case",
    "crossing_occurs",
    "first_crossing_step",
    "phase34_compatible_crossing",
    "recoverable_crossing",
    "recoverable_state",
    "new_crossing_on_baseline_non_crossing_case",
    "crossing_vr_ratio",
...
def load_phase36b_baseline_non_crossing_keys() -> set[tuple[float, float, float]]:
    ...
    with PHASE36B_RESULTS_CSV.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            if bool_from_csv(row.get("crossing_occurs")):
                continue
            keys.add(case_key(row["r0_over_target"], row["initial_velocity_angle_deg"], row["thrust_scale"]))
    return keys
...
def commit_is_active(variant: Phase37AVariant, step_index: int, diag, vr_ratio: float) -> bool:
    abs_r = abs(diag.r_error_ratio)
    if variant.commit_timing == "early_commit":
        return True
    if variant.commit_timing == "mid_commit":
        return step_index >= MAX_STEPS // 2 or abs_r <= 0.030 or orbit_crosses_target(diag, DEFAULT_TARGET_RADIUS * TARGET_RADIUS_SCALE)
    if variant.commit_timing == "delayed_commit":
        return step_index >= int(0.75 * MAX_STEPS) or (abs_r <= 0.021 and abs(vr_ratio) <= 0.025)
    raise ValueError(f"Unknown commit timing: {variant.commit_timing}")


def rollout_variant_case(
    variant: Phase37AVariant,
    mode: PostCrossMode,
    r0: float,
    angle: float,
    thrust_scale: float,
    baseline_non_crossing_keys: set[tuple[float, float, float]],
    ...
    def apply_radial_commit_action(
        state_x: float,
        state_y: float,
        state_vx: float,
        state_vy: float,
        ...
        vr_ratio = diag.radial_velocity / (v_circ + 1.0e-12)
        if not commit_is_active(variant, step_index, diag, vr_ratio):
            return base_action, f"{active_stage}_precommit"

        direction_to_target = -1.0 if diag.r_error_ratio > 0.0 else 1.0
        radial_limit = variant.radial_magnitude
        radial_cmd = clamp(radial_limit * direction_to_target - 0.16 * vr_ratio, -radial_limit, radial_limit)
        radial_x, radial_y = radial_tangential_action(state_x, state_y, state_vx, state_vy, radial_cmd, 0.0, radial_limit)
        max_norm = clamp(0.11 + radial_limit, 0.12, 0.24)
        return limit_norm(base_action[0] + radial_x, base_action[1] + radial_y, max_norm), f"{variant.name}_active"
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if phase == "DESCENT":
            ...
            if speed_ratio > SAFE_SPEED_RATIO or diag.radius < 0.42 * target_radius or diag.radius > 2.25 * target_radius:
                ...
                safe_coast_steps += 1
            (action_x, action_y), active_stage = apply_radial_commit_action(x, y, vx, vy, diag, (action_x, action_y), active_stage, step)
            stage_step += 1
    ...
    baseline_non_crossing_case = key in baseline_non_crossing_keys
    row: Dict[str, object] = {
        "controller_name": "phase37a_radial_commit_timing",
        "variant_name": variant.name,
        "commit_timing": variant.commit_timing,
        "radial_magnitude_label": variant.radial_magnitude_label,
        "radial_magnitude": variant.radial_magnitude,
        "r0_over_target": r0,
        "initial_velocity_angle_deg": angle,
        "thrust_scale": thrust_scale,
        "target_radius_scale": target_scale,
        "baseline_non_crossing_case": baseline_non_crossing_case,
        ...
        "new_crossing_on_baseline_non_crossing_case": bool(baseline_non_crossing_case and first_crossing_step is not None),
...
def aggregate(rows: Sequence[Dict[str, object]]) -> Dict[str, Dict[str, object]]:
    ...
    for variant in VARIANTS:
        subset = [row for row in rows if row.get("variant_name") == variant.name]
        ...
        new_crossing = [row for row in subset if bool_from_csv(row.get("new_crossing_on_baseline_non_crossing_case"))]
        out[variant.name] = {
            "cases": len(subset),
            "crossings": len(crossing),
            "new_crossings": len(new_crossing),
            ...
            "failure_counts": Counter(str(row.get("dominant_failure_label")) for row in subset if str(row.get("dominant_failure_label", ""))),
        }
    return out


def save_plot(stats: Dict[str, Dict[str, object]]) -> None:
    labels = [variant.name for variant in VARIANTS]
    x = np.arange(len(labels))
    fig, ax = plt.subplots(figsize=(11.0, 5.6))
    ax.bar(x - 0.27, [stats[label]["crossings"] for label in labels], 0.27, label="geometric crossings", color="#4C78A8")
    ax.bar(x, [stats[label]["new_crossings"] for label in labels], 0.27, label="new crossings on baseline non-crossing cases", color="#F58518")
    ax.bar(x + 0.27, [stats[label]["recoverable_crossings"] for label in labels], 0.27, label="recoverable crossings", color="#54A24B")
    ax.axhline(8, color="#333333", linewidth=1.0, linestyle="--", alpha=0.7, label="Phase36B baseline crossings")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=18, ha="right")
    ax.set_ylim(0, 24)
    ax.set_ylabel("cases out of 24")
    ax.set_title("Phase37A radial commitment timing sweep")
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, object]]) -> None:
    best_crossing = max(stats, key=lambda name: stats[name]["crossings"])
    best_new = max(stats, key=lambda name: stats[name]["new_crossings"])
    best_recoverable = max(stats, key=lambda name: stats[name]["recoverable_crossings"])
    total_rollouts = len(rows)
    any_overspeed = sum(item["overspeed"] for item in stats.values())
    any_instability = sum(item["instability"] for item in stats.values())
    total_new_crossings = sum(item["new_crossings"] for item in stats.values())
    ...
    for variant in VARIANTS:
        lines.append(f"| `{variant.commit_timing}` | `{variant.radial_magnitude_label}` | {variant.radial_magnitude:.3f} |")
    ...
    for variant in VARIANTS:
        item = stats[variant.name]
        sync_value = item["mean_crossing_sync"]
        sync_text = "N/A" if not math.isfinite(as_float(sync_value)) else f"{as_float(sync_value):.4f}"
        lines.append(
            f"| `{variant.name}` | {item['cases']} | {item['crossings']} | {item['new_crossings']} | "
            f"{item['phase34_compatible_crossings']} | {item['recoverable_crossings']} | {item['simulator_success_labels']} | "
            f"{item['overspeed']} | {item['instability']} | {sync_text} |"
        )

    if total_new_crossings > 0:
        interpretation = "Phase37A found at least one radial timing/magnitude setting that creates new geometric crossings on the Phase36B baseline non-crossing set."
        next_step = "Phase37B is justified as a local refinement around the winning timing and magnitude region, with Phase34 still fixed."
    else:
        interpretation = "Phase37A did not create new target-radius crossings on the Phase36B baseline non-crossing set."
        next_step = "Do not expand radial commitment timing blindly; inspect whether closest-approach metrics shifted before considering Phase37B."
    ...
    lines.extend(
        [
            ...
            "## Required Report",
            "",
            f"- Total rollout count: `{total_rollouts}`.",
            f"- Best total crossing variant: `{best_crossing}` with `{stats[best_crossing]['crossings']} / 24` crossings.",
            f"- Best new-crossing variant: `{best_new}` with `{stats[best_new]['new_crossings']} / 16` new crossings on baseline non-crossing cases.",
            f"- Best recoverable-crossing variant: `{best_recoverable}` with `{stats[best_recoverable]['recoverable_crossings']} / 24` recoverable crossings.",
            f"- Overspeed count across all variants: `{any_overspeed}`.",
            f"- Instability count across all variants: `{any_instability}`.",
            "",
            "## Interpretation",
            "",
            interpretation,
...
def benchmark() -> List[Dict[str, object]]:
    baseline_non_crossing_keys = load_phase36b_baseline_non_crossing_keys()
    cases = list(iter_cases())
    rows: List[Dict[str, object]] = []
    for variant in VARIANTS:
        for r0, angle, thrust in cases:
            row = rollout_variant_case(variant, PHASE34_TERMINAL_MODE, r0, angle, thrust, baseline_non_crossing_keys)
            rows.append(row)
            print(
                f"phase37a variant={variant.name} r0={r0:g} angle={angle:g} thrust={thrust:g} "
                f"cross={row['crossing_occurs']} new_cross={row['new_crossing_on_baseline_non_crossing_case']} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase37A fixed radial commitment timing sweep.")
    ...
    print(f"Saved Phase37A outputs to {OUTPUT_DIR}")
