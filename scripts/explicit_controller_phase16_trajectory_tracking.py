...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase16_trajectory_tracking"
...
SUMMARY_MD = OUTPUT_DIR / "phase16_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
RADIUS_OSCILLATION_PNG = OUTPUT_DIR / "radius_tracking_examples.png"
RADIAL_VELOCITY_PNG = OUTPUT_DIR / "radial_velocity_tracking_examples.png"
TRACKING_ERROR_PNG = OUTPUT_DIR / "tracking_error_examples.png"
...
PLANNER_MODES = ["burn", "coast", "event_burn", "oscillation", "trajectory_tracking", "soft_window"]
MAX_BURN_STEPS = 30
COOLDOWN_STEPS = 20
EVENT_BURN_STEPS = 10
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase15_oscillation", "oscillation"),
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase16_trajectory_tracking"]
...
def save_crossing_count_plot(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase15", "phase16"]
    names = ["baseline_soft_linear_3e4", "phase15_oscillation", "phase16_trajectory_tracking"]
    ...
    ax.set_title("Phase 16 crossing and capture comparison")
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    fig.suptitle(
        f"Phase 16 tracking r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase16_trajectory_tracking", "trajectory")
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def save_radius_oscillation_plot() -> None:
    controller = ControllerSpec("phase16_trajectory_tracking", "trajectory")
    ...
    for r0, angle, thrust in cases:
        ...
        desired_ratio = np.asarray(traj["desired_radius"], dtype=np.float64) / float(traj["target_radius"])
        ax.plot(
            time_days,
            radius_ratio,
            linewidth=1.1,
            label=(
                f"r0={r0:g}, a={angle:g}, T={thrust:g}, crossings={row['radius_crossings_total']}, "
                f"turns={row['turning_point_count']}"
            ),
        )
        ax.plot(time_days, desired_ratio, linewidth=0.8, linestyle=":", alpha=0.65)
    ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.0, label="target radius")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("radius / target")
    ax.set_title("Phase 16 radius tracking examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(RADIUS_OSCILLATION_PNG, dpi=220)
    plt.close(fig)


def save_radial_velocity_plot() -> None:
    controller = ControllerSpec("phase16_trajectory_tracking", "trajectory")
    cases = diagnostic_cases()
    fig, ax = plt.subplots(figsize=(9.2, 5.2))
    for r0, angle, thrust in cases:
        ...
        time_days = np.arange(len(traj["radial_velocity"]), dtype=np.float64) * DT / 86400.0
        radial_velocity = np.asarray(traj["radial_velocity"], dtype=np.float64) / (v_circ_trace + 1.0e-12)
        desired_vr = np.asarray(traj["desired_radial_velocity"], dtype=np.float64) / (v_circ_trace + 1.0e-12)
        ax.plot(time_days, radial_velocity, linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
        ax.plot(time_days, desired_vr, linewidth=0.8, linestyle=":", alpha=0.65)
    ax.axhline(0.0, color="#333333", linestyle="--", linewidth=1.0)
    ax.set_xlabel("time [days]")
    ax.set_ylabel("radial velocity / circular speed")
    ax.set_title("Phase 16 radial velocity tracking examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(RADIAL_VELOCITY_PNG, dpi=220)
    plt.close(fig)


def save_tracking_error_plot() -> None:
    controller = ControllerSpec("phase16_trajectory_tracking", "trajectory")
    cases = diagnostic_cases()
    fig, ax = plt.subplots(figsize=(9.2, 5.2))
    for r0, angle, thrust in cases:
        row = run_trace(controller, r0, angle, thrust)
        traj = row["trajectory"]
        time_days = np.arange(len(traj["tracking_error"]), dtype=np.float64) * DT / 86400.0
        tracking_error = np.asarray(traj["tracking_error"], dtype=np.float64)
        ax.plot(time_days, tracking_error, linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.axhline(0.0, color="#333333", linestyle="--", linewidth=1.0)
    ax.set_xlabel("time [days]")
    ax.set_ylabel("(radius - desired) / target")
    ax.set_title("Phase 16 radial tracking error examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(TRACKING_ERROR_PNG, dpi=220)
    plt.close(fig)
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    ...
    phase15 = stats["phase15_oscillation"]
    planner = stats["phase16_trajectory_tracking"]
    capture_improvement = planner["capture"] - baseline["capture"]
    success_improvement = planner["success"] - baseline["success"]
    crossing_improvement = sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "phase16_trajectory_tracking") - sum(
        bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"
    )
    phase15_rows = [row for row in rows if str(row["controller_name"]) == "phase15_oscillation"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase16_trajectory_tracking"]
    no_capture_baseline = baseline["total"] - baseline["capture"]
    no_capture_planner = planner["total"] - planner["capture"]
    overspeed_phase15 = sum(bool(row.get("overspeed", False)) for row in phase15_rows)
    overspeed_planner = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    overspeed_reduction = overspeed_phase15 - overspeed_planner
    ...
    mean_tracking_error = float(np.mean([float(row["mean_abs_tracking_error"]) for row in planner_rows])) if planner_rows else 0.0
    oscillation_created = total_turning_points > 0 or total_crossings > baseline_crossings
    ...
    lines = [
        ...
        f"| `phase15_oscillation` | {phase15['capture']} | {phase15['success']} | {phase15['near_miss']} | {phase15['total']} |",
        f"| `phase16_trajectory_tracking` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        f"- Total radius crossings: baseline `{baseline_crossings}`, Phase 16 `{total_crossings}`.",
        f"- Turning points detected in Phase 16: `{total_turning_points}`.",
        f"- No-CAPTURE cases: baseline `{no_capture_baseline}`, planner `{no_capture_planner}`.",
        f"- Overspeed terminations: Phase 15 `{overspeed_phase15}`, Phase 16 `{overspeed_planner}`.",
        f"- Overspeed reduction vs Phase 15: `{overspeed_reduction}`.",
        f"- Event counters inherited from scaffold: events `{total_events}`, burns `{total_burns}`.",
        f"- Mean max speed ratio: `{mean_max_speed:.4f}`.",
        f"- Mean absolute tracking error ratio: `{mean_tracking_error:.4f}`.",
        ...
        "## Answers",
        "",
        f"1. Does explicit trajectory construction create crossings? `{'yes' if crossing_improvement > 0 or total_crossings > baseline_crossings else 'no'}`. Total crossings change from `{baseline_crossings}` to `{total_crossings}`.",
        f"2. Does capture increase? It `{honest_result}` CAPTURE: CAPTURE changes by `{capture_improvement}` versus baseline.",
        f"3. Does turning point count increase? `{'yes' if total_turning_points > 0 else 'no'}`. Turning points observed: `{total_turning_points}`.",
        f"4. Does tracking stay bounded without overspeed? `{'yes' if overspeed_planner == 0 else 'no'}`. Phase 16 overspeed count is `{overspeed_planner}`.",
        f"5. Is trajectory design more effective than direct control? `{'partially on boundedness, not on reachability' if overspeed_planner < overspeed_phase15 and planner['capture'] <= phase15['capture'] else 'yes' if planner['capture'] > phase15['capture'] else 'no'}`. The decisive CAPTURE change is `{capture_improvement}` versus baseline and `{planner['capture'] - phase15['capture']}` versus Phase 15.",
        "",
        "## Main Limitation",
        "",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase16 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
