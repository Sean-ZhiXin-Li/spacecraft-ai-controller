...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase75_hybrid"
...
GRID_CSV_PATH = OUTPUT_DIR / "hybrid_grid.csv"
RANKING_CSV_PATH = OUTPUT_DIR / "hybrid_ranking.csv"
SUMMARY_PATH = OUTPUT_DIR / "phase75_summary.md"
PHASE7_GRID_CSV_PATH = PROJECT_ROOT / "analysis" / "phase7_pre_window_shaping" / "pre_window_grid.csv"

BASELINE_PNG = OUTPUT_DIR / "hybrid_vs_baseline.png"
SUCCESS_MAP_PNG = OUTPUT_DIR / "hybrid_success_map.png"
...
def define_variants() -> List[HybridVariant]:
    return [
        HybridVariant("hybrid_1e4", 1.0e-4, dict(BASE_PARAMS)),
        HybridVariant("hybrid_2e4", 2.0e-4, dict(BASE_PARAMS)),
        HybridVariant("hybrid_3e4", 3.0e-4, dict(BASE_PARAMS)),
    ]
...
def rollout_case(variant: HybridVariant, r0: float, angle: float, thrust_scale: float, target_scale: float) -> Dict[str, object]:
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if phase == "DESCENT":
            if abs_r_ratio > variant.switch_threshold:
                action_x, action_y = prewindow_radial_medium_action(x, y, vx, vy, radius, real_r_ratio)
            else:
                action_x, action_y = adaptive_soft_ws_action(x, y, vx, vy, radius, abs_r_ratio)
        else:
    ...
    return {
        ...
        "variant_params_json": json.dumps({**variant.params, "switch_threshold": variant.switch_threshold}, sort_keys=True),
...
def load_rows(path: Path = GRID_CSV_PATH) -> List[Dict[str, object]]:
    ...
    for row in loaded:
        rows.append(
            {
                **row,
                "switch_threshold": float(row["switch_threshold"]),
...
def load_phase7_reference_rows() -> List[Dict[str, object]]:
    if not PHASE7_GRID_CSV_PATH.exists():
        return []
    with PHASE7_GRID_CSV_PATH.open("r", newline="", encoding="utf-8") as f:
        loaded = list(csv.DictReader(f))
    rows: List[Dict[str, object]] = []
    for row in loaded:
        if row["variant_name"] not in {"adaptive_soft", "prewindow_radial_medium"}:
            continue
        rows.append(
            {
                **row,
...
def rank_rows(rows: Sequence[Dict[str, object]], variants: Sequence[HybridVariant]) -> List[Dict[str, object]]:
    ranking: List[Dict[str, object]] = []
    by_threshold = {variant.name: variant.switch_threshold for variant in variants}
    for row in aggregate_rows(rows, [variant.name for variant in variants]):
        row["switch_threshold"] = by_threshold[str(row["variant_name"])]
        ranking.append(row)
    ranking.sort(
        key=lambda row: (
...
def save_plots(hybrid_rows: Sequence[Dict[str, object]], ranking: Sequence[Dict[str, object]], reference_rows: Sequence[Dict[str, object]]) -> None:
    ...
    comparison_names = ["adaptive_soft", "prewindow_radial_medium", best_hybrid_name]
    ...
    ax.set_title("Hybrid vs adaptive_soft and prewindow_radial_medium")
...
def write_summary(hybrid_rows: Sequence[Dict[str, object]], ranking: Sequence[Dict[str, object]], reference_rows: Sequence[Dict[str, object]]) -> None:
    refs = {row["variant_name"]: row for row in aggregate_rows(reference_rows, ["adaptive_soft", "prewindow_radial_medium"])}
    ...
    if maintains_high and reduces_near:
        tradeoff_text = "the hybrid combines the desired strengths on this grid"
    elif maintains_high:
        tradeoff_text = "the hybrid preserves reachability but still trades off near-miss stability"
    elif reduces_near:
        tradeoff_text = "the hybrid improves near-miss stability but gives up reachability"
    else:
        tradeoff_text = "the hybrid does not beat the Phase 7 reachability/stability trade-off"
    ...
    for row in ranking:
        lines.append(
            f"- `{row['variant_name']}` threshold `{float(row['switch_threshold']):.1e}`: success `{row['success_count']}`, CAPTURE `{row['capture_count']}`, near-miss `{row['near_miss_count']}`."
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            f"- Best hybrid: `{best['variant_name']}` with success `{best['success_count']}`, CAPTURE `{best['capture_count']}`, near-miss `{best['near_miss_count']}`.",
            "",
            "## Answers",
            "",
            f"1. Does hybrid maintain the high success of prewindow_radial_medium? `{'yes' if maintains_high else 'no'}`. Delta vs prewindow_radial_medium is success `{success_delta_pre:+d}`.",
            f"2. Does it reduce near-miss? `{'yes' if reduces_near else 'no'}`. Delta vs prewindow_radial_medium is near-miss `{near_delta_pre:+d}`; delta vs adaptive_soft is `{near_delta_adaptive:+d}`.",
            f"3. Which switch_threshold works best? `{best['variant_name']}` with threshold `{float(best['switch_threshold']):.1e}`.",
            f"4. Does hybrid truly combine strengths, or still trade off? `{tradeoff_text}`. Success overlap vs prewindow is retained `{len(best_cases & pre_cases)}`, gained `{len(best_cases - pre_cases)}`, lost `{len(pre_cases - best_cases)}`; overlap vs adaptive is retained `{len(best_cases & adaptive_cases)}`, gained `{len(best_cases - adaptive_cases)}`, lost `{len(adaptive_cases - best_cases)}`.",
...
def iter_cases(variants: Sequence[HybridVariant]) -> Iterable[tuple[HybridVariant, float, float, float, float]]:
...
def main() -> None:
    ...
    if pending:
        ...
        print(f"phase75 pending={len(pending)} completed={len(done)} total={total} workers={workers}")
        if workers == 1:
            for idx, (variant, r0, angle, thrust, target_scale) in pending:
                ...
                print(
                    f"phase75 {idx}/{total} variant={variant.name} r0={r0:.5f} angle={angle:g} "
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                ...
                for future in as_completed(future_to_case):
                    ...
                    completed_now += 1
                    print(
                        f"phase75 {idx}/{total} done_now={completed_now}/{len(pending)} variant={variant.name} "
        ...
        print(f"phase75 no pending rows; completed={len(done)} total={total}")

    rows = load_rows()
    ranking = rank_rows(rows, variants)
    reference_rows = load_phase7_reference_rows()
    write_ranking(ranking)
    save_plots(rows, ranking, reference_rows)
    write_summary(rows, ranking, reference_rows)
    print(f"Saved Phase 7.5 hybrid outputs to {OUTPUT_DIR}")
