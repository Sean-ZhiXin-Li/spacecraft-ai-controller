    ...
    for command, label in (
        ...
        if subprocess.run(["git", *command], cwd=ROOT, check=False).returncode:
            raise PredictionBoundaryDiscoveryError(
                f"frozen discovery blocked by {label}"
            )
    head = _git_output("rev-parse", "HEAD")
    origin = _git_output("rev-parse", "origin/main")
    if not head or head != origin:
        raise PredictionBoundaryDiscoveryError(
            "frozen discovery requires the implementation HEAD pushed to origin/main"
        )
    return head
...
def _plan() -> None:
    ...
    print(f"source_case_count={len(plan['source_case_ids'])}")
    print(f"branches={','.join(DISCOVERY_BRANCH_IDS)}")
    print(f"planned_trajectory_count={PLANNED_TRAJECTORY_COUNT}")
    print(
        "maximum_physical_transitions_per_trajectory="
        f"{MAXIMUM_PHYSICAL_TRANSITIONS_PER_TRAJECTORY}"
    )
    print(f"discovery_plan_hash={plan['canonical_plan_hash']}")
...
def _execute() -> None:
    ...
    prefixes, trajectories = execute_frozen_discovery(
        ROOT,
    ...
    target = atomic_publish_new_directory(
        ...
        OUTPUT_PATH,
        payloads,
        lambda values: validate_discovery_payloads(values, source_plan=plan),
    )
    ...
    for key in (
        "planned_trajectory_count",
        "started_trajectory_count",
        "completed_trajectory_count",
        "prefix_physical_transition_count",
        "normal_branch_physical_transition_count",
        "total_physical_transition_count",
        "states_evaluated",
        "Final_Veto_rejection_count",
        "candidate_boundary_count",
        "hazard_arrest_interventions",
    ):
        print(f"{key}={manifest[key]}")
    print(
        "normal_branch_execution_counts="
        + json.dumps(
            coverage["normal_branch_execution_counts"],
            sort_keys=True,
            separators=(",", ":"),
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Frozen Stage 2A prediction-boundary discovery. Default invocation "
            "executes no transition and writes nothing."
        )
    )
