...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase13_burn_coast"
...
SUMMARY_MD = OUTPUT_DIR / "phase13_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
BURN_SCHEDULE_PNG = OUTPUT_DIR / "burn_schedule.png"
SPEED_VS_TIME_PNG = OUTPUT_DIR / "speed_vs_time.png"
ENERGY_VS_TIME_PNG = OUTPUT_DIR / "energy_vs_time.png"
...
PLANNER_MODES = ["orbit_intersection", "burn", "coast", "cooldown", "soft_window"]
MAX_BURN_STEPS = 30
COOLDOWN_STEPS = 20
...
def rollout_case(
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if record_trajectory:
            ...
            trajectory_burn_flag.append(1 if planner_mode == "burn" else 0)
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase13_burn_coast"]
...
def save_capture_improvement(stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase12", "phase13"]
    baseline = stats["baseline_soft_linear_3e4"]
    orbit = stats["phase12_orbit_planner"]
    burn = stats["phase13_burn_coast"]
    captures = [baseline["capture"], orbit["capture"], burn["capture"]]
    successes = [baseline["success"], orbit["success"], burn["success"]]
    ...
    ax.set_title("Phase 13 burn-coast comparison")
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    fig.suptitle(
        f"Phase 13 burn-coast r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase13_burn_coast", "burn_coast")
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def save_burn_schedule_plot() -> None:
    controller = ControllerSpec("phase13_burn_coast", "burn_coast")
    ...
    for idx, (r0, angle, thrust) in enumerate(cases):
        ...
        ax.step(time_days, burn_flag + idx * 1.25, where="post", linewidth=0.9, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("burn flag + case offset")
    ax.set_title("Phase 13 burn/coast schedule examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(BURN_SCHEDULE_PNG, dpi=220)
    plt.close(fig)


def save_speed_vs_time_plot() -> None:
    controller = ControllerSpec("phase13_burn_coast", "burn_coast")
    cases = [
        (0.98, 150.0, 8000.0),
        (1.00, 170.0, 10000.0),
    ...
    for r0, angle, thrust in cases:
        row = run_trace(controller, r0, angle, thrust)
        traj = row["trajectory"]
        time_days = np.arange(len(traj["speed_ratio"]), dtype=np.float64) * DT / 86400.0
        speed_ratio = np.asarray(traj["speed_ratio"], dtype=np.float64)
        ax.plot(time_days, speed_ratio, linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.axhline(1.9, color="#A50F15", linestyle="--", linewidth=1.0, label="overspeed limit")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("speed / circular speed")
    ax.set_title("Phase 13 speed evolution examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(SPEED_VS_TIME_PNG, dpi=220)
    plt.close(fig)


def save_energy_vs_time_plot() -> None:
    controller = ControllerSpec("phase13_burn_coast", "burn_coast")
    cases = diagnostic_cases()
    fig, ax = plt.subplots(figsize=(9.2, 5.2))
    for r0, angle, thrust in cases:
        ...
        energy_ratio = np.asarray(traj["energy"], dtype=np.float64) / abs(float(traj["target_energy"]))
        ax.plot(time_days, energy_ratio, linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.axhline(-1.0, color="#333333", linestyle="--", linewidth=1.0, label="target energy")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("specific energy / |target energy|")
    ax.set_title("Phase 13 energy evolution examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(ENERGY_VS_TIME_PNG, dpi=220)
    plt.close(fig)


def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    baseline = stats["baseline_soft_linear_3e4"]
    phase12 = stats["phase12_orbit_planner"]
    planner = stats["phase13_burn_coast"]
    capture_improvement = planner["capture"] - baseline["capture"]
    success_improvement = planner["success"] - baseline["success"]
    crossing_improvement = sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "phase13_burn_coast") - sum(
        bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"
    )
    phase12_rows = [row for row in rows if str(row["controller_name"]) == "phase12_orbit_planner"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase13_burn_coast"]
    no_capture_baseline = baseline["total"] - baseline["capture"]
    no_capture_planner = planner["total"] - planner["capture"]
    overspeed_phase12 = sum(str(row["termination_reason"]) == "overspeed" for row in phase12_rows)
    overspeed_planner = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    overspeed_reduction = overspeed_phase12 - overspeed_planner
    ...
    lines = [
        ...
        f"| `phase12_orbit_planner` | {phase12['capture']} | {phase12['success']} | {phase12['near_miss']} | {phase12['total']} |",
        f"| `phase13_burn_coast` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        f"- Overspeed terminations: Phase 12 `{overspeed_phase12}`, Phase 13 `{overspeed_planner}`.",
        f"- Overspeed reduction vs Phase 12: `{overspeed_reduction}`.",
        ...
        "## Answers",
        "",
        f"1. Does burn-coast reduce overspeed? `{'yes' if overspeed_reduction > 0 else 'no'}`. Overspeed changes from `{overspeed_phase12}` to `{overspeed_planner}`.",
        f"2. Does capture increase? It `{honest_result}` CAPTURE: CAPTURE changes by `{capture_improvement}` versus baseline.",
        f"3. Does trajectory look more structured? `{'yes' if mean_burn_ratio < 0.95 else 'not much'}`. The burn/coast ratio is `{mean_burn_ratio:.4f}`, and the schedule plots show explicit coast intervals.",
        f"4. Is timing now the key missing factor? `{'partially' if overspeed_reduction > 0 and capture_improvement <= 0 else 'yes' if capture_improvement > 0 else 'not by itself'}`. Timing helps only if it also produces target-radius crossing and CAPTURE.",
        "",
        "## Artifacts",
        "",
        "- `comparison.csv`",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase13 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
            ...
            f"burn_ratio={row['burn_ratio']:.4f}"
        )
    ...
    save_burn_schedule_plot()
    save_speed_vs_time_plot()
    save_energy_vs_time_plot()
    write_summary(rows, stats, planner_diagnostics)
    print(f"Saved Phase 13 burn-coast outputs to {OUTPUT_DIR}")
