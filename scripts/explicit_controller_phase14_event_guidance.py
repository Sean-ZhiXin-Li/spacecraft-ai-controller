...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase14_event_guidance"
...
SUMMARY_MD = OUTPUT_DIR / "phase14_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
EVENT_TIMELINE_PNG = OUTPUT_DIR / "event_timeline.png"
RADIUS_EVENTS_PNG = OUTPUT_DIR / "radius_vs_time_with_events.png"
...
PLANNER_MODES = ["burn", "coast", "event_burn", "soft_window"]
MAX_BURN_STEPS = 30
COOLDOWN_STEPS = 20
EVENT_BURN_STEPS = 10
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase13_burn_coast", "burn_coast"),
        ...
        if phase == "DESCENT":
            if controller.planner_kind == "orbit":
                ...
                if abs_r_ratio <= PREWINDOW_OUTER_RATIO:
                    ...
                    turning_point = prev_vr_for_event is not None and (prev_vr_for_event * vr_current < 0.0)
                    ...
                    event_detected = turning_point or near_miss_event
                    if event_detected:
                        event_count += 1
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase14_event_guidance"]
...
def save_capture_improvement(stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase13", "phase14"]
    baseline = stats["baseline_soft_linear_3e4"]
    burn = stats["phase13_burn_coast"]
    event = stats["phase14_event_guidance"]
    captures = [baseline["capture"], burn["capture"], event["capture"]]
    successes = [baseline["success"], burn["success"], event["success"]]
    ...
    ax.set_title("Phase 14 event guidance comparison")
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    fig.suptitle(
        f"Phase 14 event guidance r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase14_event_guidance", "event")
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def save_event_timeline_plot() -> None:
    controller = ControllerSpec("phase14_event_guidance", "event")
    cases = diagnostic_cases()
    fig, ax = plt.subplots(figsize=(9.2, 5.2))
    for idx, (r0, angle, thrust) in enumerate(cases):
        row = run_trace(controller, r0, angle, thrust)
        traj = row["trajectory"]
        time_days = np.arange(len(traj["burn_flag"]), dtype=np.float64) * DT / 86400.0
        burn_flag = np.asarray(traj["burn_flag"], dtype=np.float64)
        event_flag = np.asarray(traj["event_flag"], dtype=np.float64)
        offset = idx * 1.45
        ax.step(time_days, burn_flag + offset, where="post", linewidth=0.9, label=f"burn r0={r0:g}, a={angle:g}, T={thrust:g}")
        event_times = time_days[event_flag > 0.5]
        if len(event_times):
            ax.scatter(event_times, np.full_like(event_times, offset + 1.12), s=10, color="#D95F02")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("event / burn flags + case offset")
    ax.set_title("Phase 14 event-triggered burn timeline")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(EVENT_TIMELINE_PNG, dpi=220)
    plt.close(fig)


def save_radius_events_plot() -> None:
    controller = ControllerSpec("phase14_event_guidance", "event")
    cases = [
        (0.98, 150.0, 8000.0),
        (1.00, 170.0, 10000.0),
    ...
    for r0, angle, thrust in cases:
        ...
        event_flag = np.asarray(traj["event_flag"], dtype=np.float64)
        ax.plot(time_days, radius_ratio, linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}, cap={row['capture_entered']}")
        event_times = time_days[event_flag > 0.5]
        event_radii = radius_ratio[event_flag > 0.5]
        if len(event_times):
            ax.scatter(event_times, event_radii, s=12, color="#D95F02")
    ax.axhline(1.0, color="#333333", linestyle="--", linewidth=1.0, label="target radius")
    ax.set_xlabel("time [days]")
    ax.set_ylabel("radius / target")
    ax.set_title("Phase 14 radius trajectories with event markers")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(RADIUS_EVENTS_PNG, dpi=220)
    plt.close(fig)


def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    baseline = stats["baseline_soft_linear_3e4"]
    phase13 = stats["phase13_burn_coast"]
    planner = stats["phase14_event_guidance"]
    capture_improvement = planner["capture"] - baseline["capture"]
    success_improvement = planner["success"] - baseline["success"]
    crossing_improvement = sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "phase14_event_guidance") - sum(
        bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"
    )
    phase13_rows = [row for row in rows if str(row["controller_name"]) == "phase13_burn_coast"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase14_event_guidance"]
    no_capture_baseline = baseline["total"] - baseline["capture"]
    no_capture_planner = planner["total"] - planner["capture"]
    overspeed_phase13 = sum(str(row["termination_reason"]) == "overspeed" for row in phase13_rows)
    overspeed_planner = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    overspeed_reduction = overspeed_phase13 - overspeed_planner
    ...
    mean_burn_ratio = float(np.mean([float(row["burn_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    ...
    lines = [
        ...
        f"| `phase13_burn_coast` | {phase13['capture']} | {phase13['success']} | {phase13['near_miss']} | {phase13['total']} |",
        f"| `phase14_event_guidance` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        f"- Overspeed terminations: Phase 13 `{overspeed_phase13}`, Phase 14 `{overspeed_planner}`.",
        f"- Overspeed reduction vs Phase 13: `{overspeed_reduction}`.",
        f"- Events detected: `{total_events}`.",
        f"- Burns started: `{total_burns}`.",
        f"- Mean burn ratio: `{mean_burn_ratio:.4f}`.",
        ...
        "## Answers",
        "",
        f"1. Does event-triggered burn improve crossing? `{'yes' if crossing_improvement > 0 else 'no'}`. Crossing count changes by `{crossing_improvement}` versus baseline.",
        f"2. Does capture increase? It `{honest_result}` CAPTURE: CAPTURE changes by `{capture_improvement}` versus baseline.",
        f"3. Does trajectory now resemble orbital transfer? `{'partially' if total_events > 0 and mean_burn_ratio < 0.5 else 'not yet'}`. The controller uses event-triggered coast/burn timing, but the measured crossing/CAPTURE result is still the decisive test.",
        f"4. Is event structure the missing piece? `{'partially' if overspeed_reduction > 0 and capture_improvement <= 0 else 'yes' if capture_improvement > 0 else 'not by itself'}`. Event structure reduces uncontrolled thrusting only if events occur early enough and burns are sized correctly.",
        "",
        "## Artifacts",
        "",
        "- `comparison.csv`",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase14 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
    ...
    save_event_timeline_plot()
    save_radius_events_plot()
    write_summary(rows, stats, planner_diagnostics)
    print(f"Saved Phase 14 event-guidance outputs to {OUTPUT_DIR}")
