...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase17_elliptical_orbit"
...
SUMMARY_MD = OUTPUT_DIR / "phase17_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
ENERGY_CONVERGENCE_PNG = OUTPUT_DIR / "energy_convergence.png"
ANGULAR_MOMENTUM_CONVERGENCE_PNG = OUTPUT_DIR / "angular_momentum_convergence.png"
ORBIT_SHAPE_PNG = OUTPUT_DIR / "orbit_shape_examples.png"
CROSSING_COUNT_PNG = OUTPUT_DIR / "crossing_count_comparison.png"
...
PLANNER_MODES = ["burn", "coast", "event_burn", "oscillation", "trajectory_tracking", "elliptical_guidance", "soft_window"]
MAX_BURN_STEPS = 30
COOLDOWN_STEPS = 20
EVENT_BURN_STEPS = 10
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase16_trajectory_tracking", "trajectory"),
        ControllerSpec("phase17_elliptical_orbit", "elliptical"),
    ]
...
def rollout_case(
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if record_trajectory:
            ...
            trajectory_burn_flag.append(1 if planner_mode in {"burn", "event_burn"} else 0)
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase17_elliptical_orbit"]
...
def save_crossing_count_plot(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase16", "phase17"]
    names = ["baseline_soft_linear_3e4", "phase16_trajectory_tracking", "phase17_elliptical_orbit"]
    crossing_cases = [sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == name) for name in names]
    total_crossings = [sum(int(row["radius_crossings_total"]) for row in rows if str(row["controller_name"]) == name) for name in names]
    captures = [stats[name]["capture"] for name in names]
    x = np.arange(len(labels))
    fig, ax = plt.subplots(figsize=(7.6, 4.8))
    ax.bar(x - 0.24, crossing_cases, width=0.24, label="crossing cases", color="#7570B3")
    ax.bar(x, total_crossings, width=0.24, label="total crossings", color="#D95F02")
    ax.bar(x + 0.24, captures, width=0.24, label="CAPTURE", color="#386CB0")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("count over 48 regimes")
    ax.set_title("Phase 17 crossing and capture comparison")
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend()
    for values, offset in [(crossing_cases, -0.24), (total_crossings, 0.0), (captures, 0.24)]:
        for idx, value in enumerate(values):
            ax.text(idx + offset, value + 0.4, str(value), ha="center", va="bottom", fontsize=8)
    fig.tight_layout()
    fig.savefig(CROSSING_COUNT_PNG, dpi=220)
    plt.close(fig)
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    fig.suptitle(
        f"Phase 17 elliptical guidance r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase17_elliptical_orbit", "elliptical")
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def save_energy_convergence_plot() -> None:
    controller = ControllerSpec("phase17_elliptical_orbit", "elliptical")
    ...
    for r0, angle, thrust in cases:
        ...
        traj = row["trajectory"]
        time_days = np.arange(len(traj["energy"]), dtype=np.float64) * DT / 86400.0
        energy = np.asarray(traj["energy"], dtype=np.float64)
        transfer_energy = float(traj["transfer_energy"])
        ax.plot(time_days, (energy - transfer_energy) / abs(transfer_energy), linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.axhline(0.0, color="#333333", linestyle="--", linewidth=1.0)
    ax.set_xlabel("time [days]")
    ax.set_ylabel("(E - E_transfer) / |E_transfer|")
    ax.set_title("Phase 17 transfer-energy convergence")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(ENERGY_CONVERGENCE_PNG, dpi=220)
    plt.close(fig)
...
def save_angular_momentum_plot() -> None:
    ...
    for r0, angle, thrust in cases:
        ...
        angular_momentum = np.asarray(traj["angular_momentum"], dtype=np.float64)
        transfer_l = float(traj["transfer_angular_momentum"])
        ax.plot(time_days, (angular_momentum - transfer_l) / abs(transfer_l), linewidth=1.1, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    ax.axhline(0.0, color="#333333", linestyle="--", linewidth=1.0)
    ax.set_xlabel("time [days]")
    ax.set_ylabel("(L - L_transfer) / |L_transfer|")
    ax.set_title("Phase 17 angular-momentum convergence")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(ANGULAR_MOMENTUM_CONVERGENCE_PNG, dpi=220)
    plt.close(fig)


def save_orbit_shape_plot() -> None:
    controller = ControllerSpec("phase17_elliptical_orbit", "elliptical")
    cases = diagnostic_cases()
    ...
    for r0, angle, thrust in cases:
        ...
        traj = row["trajectory"]
        target_radius = float(traj["target_radius"])
        x = np.asarray(traj["x"], dtype=np.float64) / target_radius
        y = np.asarray(traj["y"], dtype=np.float64) / target_radius
        stride = max(1, len(x) // 3000)
        ax.plot(x[::stride], y[::stride], linewidth=1.0, label=f"r0={r0:g}, a={angle:g}, T={thrust:g}")
    theta = np.linspace(0.0, 2.0 * math.pi, 360)
    ax.plot(np.cos(theta), np.sin(theta), color="#333333", linestyle="--", linewidth=1.0, label="target radius")
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("x / target_radius")
    ax.set_ylabel("y / target_radius")
    ax.set_title("Phase 17 orbit-shape examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(ORBIT_SHAPE_PNG, dpi=220)
    plt.close(fig)
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    ...
    phase16 = stats["phase16_trajectory_tracking"]
    planner = stats["phase17_elliptical_orbit"]
    capture_improvement = planner["capture"] - baseline["capture"]
    success_improvement = planner["success"] - baseline["success"]
    crossing_improvement = sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "phase17_elliptical_orbit") - sum(
        bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"
    )
    phase16_rows = [row for row in rows if str(row["controller_name"]) == "phase16_trajectory_tracking"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase17_elliptical_orbit"]
    no_capture_baseline = baseline["total"] - baseline["capture"]
    no_capture_planner = planner["total"] - planner["capture"]
    overspeed_phase16 = sum(bool(row.get("overspeed", False)) for row in phase16_rows)
    overspeed_planner = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    overspeed_reduction = overspeed_phase16 - overspeed_planner
    total_events = sum(int(float(row["event_count"])) for row in planner_rows)
    total_burns = sum(int(float(row["burn_count"])) for row in planner_rows)
    total_turning_points = sum(int(float(row["turning_point_count"])) for row in planner_rows)
    total_crossings = sum(int(float(row["radius_crossings_total"])) for row in planner_rows)
    baseline_crossings = sum(int(float(row["radius_crossings_total"])) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4")
    ...
    mean_max_speed = float(np.mean([float(row["max_speed_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    mean_transfer_e0 = float(np.mean([float(row["initial_transfer_energy_error_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    mean_transfer_ef = float(np.mean([float(row["final_transfer_energy_error_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    mean_transfer_l0 = float(np.mean([float(row["initial_transfer_angular_momentum_error_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    mean_transfer_lf = float(np.mean([float(row["final_transfer_angular_momentum_error_ratio"]) for row in planner_rows])) if planner_rows else 0.0
    real_orbit_shape = mean_transfer_ef < mean_transfer_e0 and mean_transfer_lf < mean_transfer_l0
    ...
    lines = [
        ...
        f"| `phase16_trajectory_tracking` | {phase16['capture']} | {phase16['success']} | {phase16['near_miss']} | {phase16['total']} |",
        f"| `phase17_elliptical_orbit` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        f"- Capture improvement: `{capture_improvement}`.",
        f"- Success improvement: `{success_improvement}`.",
        f"- Crossing-case improvement: `{crossing_improvement}`.",
        f"- Total radius crossings: baseline `{baseline_crossings}`, Phase 17 `{total_crossings}`.",
        f"- Turning points detected in Phase 17: `{total_turning_points}`.",
        f"- No-CAPTURE cases: baseline `{no_capture_baseline}`, planner `{no_capture_planner}`.",
        f"- Overspeed terminations: Phase 16 `{overspeed_phase16}`, Phase 17 `{overspeed_planner}`.",
        f"- Overspeed reduction vs Phase 16: `{overspeed_reduction}`.",
        f"- Event counters inherited from scaffold: events `{total_events}`, burns `{total_burns}`.",
        f"- Mean max speed ratio: `{mean_max_speed:.4f}`.",
        ...
        f"- Mean transfer energy error ratio: initial `{mean_transfer_e0:.4f}`, final `{mean_transfer_ef:.4f}`.",
        f"- Mean transfer angular-momentum error ratio: initial `{mean_transfer_l0:.4f}`, final `{mean_transfer_lf:.4f}`.",
        f"- Planner mode usage: `{json.dumps(planner_diagnostics.get('descent_step_counts', {}), sort_keys=True)}`.",
        "",
        "## Answers",
        "",
        f"1. Does physics-consistent orbit improve capture? It `{honest_result}` CAPTURE: CAPTURE changes by `{capture_improvement}` versus baseline.",
        f"2. Does turning point count increase? `{'yes' if total_turning_points > 0 else 'no'}`. Turning points observed: `{total_turning_points}`.",
        f"3. Does system now form real orbital shape? `{'partially' if real_orbit_shape else 'not reliably'}`. Transfer energy/angular-momentum errors changed from `{mean_transfer_e0:.4f}/{mean_transfer_l0:.4f}` to `{mean_transfer_ef:.4f}/{mean_transfer_lf:.4f}`.",
        f"4. Is feasibility the missing piece? `partially`. The target is physically consistent, but the controller must still phase the transfer so target-radius crossing occurs inside the CAPTURE window.",
        "",
        "## Main Limitation",
        "",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase17 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
            ...
            f"burn_ratio={row['burn_ratio']:.4f} events={row['event_count']}"
        )
    write_comparison(rows)
    stats = controller_stats(rows)
    planner_diagnostics = write_planner_diagnostics(rows)
    save_crossing_count_plot(rows, stats)
    save_trajectory_diagnostics()
    save_energy_convergence_plot()
    save_angular_momentum_plot()
    save_orbit_shape_plot()
    write_summary(rows, stats, planner_diagnostics)
    print(f"Saved Phase 17 elliptical-orbit outputs to {OUTPUT_DIR}")
