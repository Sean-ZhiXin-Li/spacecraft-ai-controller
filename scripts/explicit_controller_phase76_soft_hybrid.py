...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase76_soft_hybrid"
...
GRID_CSV_PATH = OUTPUT_DIR / "soft_hybrid_grid.csv"
RANKING_CSV_PATH = OUTPUT_DIR / "soft_hybrid_ranking.csv"
SUMMARY_PATH = OUTPUT_DIR / "phase76_summary.md"
PHASE7_GRID_CSV_PATH = PROJECT_ROOT / "analysis" / "phase7_pre_window_shaping" / "pre_window_grid.csv"
PHASE75_GRID_CSV_PATH = PROJECT_ROOT / "analysis" / "phase75_hybrid" / "hybrid_grid.csv"

BASELINE_PNG = OUTPUT_DIR / "soft_hybrid_comparison.png"
SUCCESS_MAP_PNG = OUTPUT_DIR / "soft_hybrid_success_map.png"
...
def define_variants() -> List[SoftHybridVariant]:
    variants = [
        SoftHybridVariant("soft_linear_2e4", "linear", 2.0e-4, 0.0, dict(BASE_PARAMS)),
        SoftHybridVariant("soft_linear_3e4", "linear", 3.0e-4, 0.0, dict(BASE_PARAMS)),
        SoftHybridVariant("soft_linear_5e4", "linear", 5.0e-4, 0.0, dict(BASE_PARAMS)),
        SoftHybridVariant("soft_sigmoid_mid2e4", "sigmoid", 2.0e-4, 5.0e-5, dict(BASE_PARAMS)),
        SoftHybridVariant("soft_sigmoid_mid3e4", "sigmoid", 3.0e-4, 7.0e-5, dict(BASE_PARAMS)),
    ]
    if len(variants) > 8:
        raise RuntimeError(f"Phase 7.6 variant cap exceeded: {len(variants)}")
    return variants
...
def rollout_case(variant: SoftHybridVariant, r0: float, angle: float, thrust_scale: float, target_scale: float) -> Dict[str, object]:
    ...
    def alpha_value(abs_r_error_ratio: float) -> float:
        if variant.alpha_schedule == "linear":
            return clamp(abs_r_error_ratio / (variant.alpha_mid + 1.0e-12), 0.0, 1.0)
        if variant.alpha_schedule == "sigmoid":
            z = (abs_r_error_ratio - variant.alpha_mid) / (variant.alpha_width + 1.0e-12)
            z = clamp(z, -60.0, 60.0)
            return 1.0 / (1.0 + math.exp(-z))
        raise ValueError(f"Unknown alpha schedule: {variant.alpha_schedule}")
    ...
    return {
        ...
        "variant_params_json": json.dumps(
            {
                **variant.params,
                "alpha_schedule": variant.alpha_schedule,
                "alpha_mid": variant.alpha_mid,
                "alpha_width": variant.alpha_width,
            },
            sort_keys=True,
        ),
        "alpha_schedule": variant.alpha_schedule,
...
def load_rows(path: Path = GRID_CSV_PATH) -> List[Dict[str, object]]:
    ...
    for row in loaded:
        rows.append(
            {
                **row,
                "alpha_mid": float(row["alpha_mid"]),
                "alpha_width": float(row["alpha_width"]),
...
def load_reference_rows() -> List[Dict[str, object]]:
    rows: List[Dict[str, object]] = []
    sources = [
        (PHASE7_GRID_CSV_PATH, {"adaptive_soft", "prewindow_radial_medium"}, {}),
        (PHASE75_GRID_CSV_PATH, {"hybrid_1e4"}, {"hybrid_1e4": "hard_hybrid_1e4"}),
    ]
    for path, names, rename in sources:
        if not path.exists():
            continue
        with path.open("r", newline="", encoding="utf-8") as f:
            loaded = list(csv.DictReader(f))
        for row in loaded:
            if row["variant_name"] not in names:
                continue
            rows.append(
                {
                    **row,
                    "variant_name": rename.get(row["variant_name"], row["variant_name"]),
                    "r0_over_target": float(row["r0_over_target"]),
                    "initial_velocity_angle_deg": float(row["initial_velocity_angle_deg"]),
                    "thrust_scale": float(row["thrust_scale"]),
                    "target_radius_scale": float(row["target_radius_scale"]),
...
def aggregate_rows(rows: Sequence[Dict[str, object]], variant_names: Sequence[str] | None = None) -> List[Dict[str, object]]:
    names = list(variant_names) if variant_names is not None else sorted({str(row["variant_name"]) for row in rows})
...
def rank_rows(rows: Sequence[Dict[str, object]], variants: Sequence[SoftHybridVariant]) -> List[Dict[str, object]]:
    ranking: List[Dict[str, object]] = []
    variant_by_name = {variant.name: variant for variant in variants}
    for row in aggregate_rows(rows, [variant.name for variant in variants]):
        variant = variant_by_name[str(row["variant_name"])]
        row["alpha_schedule"] = variant.alpha_schedule
        row["alpha_mid"] = variant.alpha_mid
        row["alpha_width"] = variant.alpha_width
        ranking.append(row)
    ranking.sort(
        key=lambda row: (
...
def save_plots(hybrid_rows: Sequence[Dict[str, object]], ranking: Sequence[Dict[str, object]], reference_rows: Sequence[Dict[str, object]]) -> None:
    best_hybrid_name = str(ranking[0]["variant_name"])
    combined_rows = list(reference_rows) + list(hybrid_rows)
    comparison_names = ["adaptive_soft", "prewindow_radial_medium", "hard_hybrid_1e4", best_hybrid_name]
    comparison = aggregate_rows(combined_rows, comparison_names)

    x = np.arange(len(comparison_names))
    success = [next(row for row in comparison if row["variant_name"] == name)["success_count"] for name in comparison_names]
    near_miss = [next(row for row in comparison if row["variant_name"] == name)["near_miss_count"] for name in comparison_names]
    width = 0.36
    fig, ax = plt.subplots(figsize=(8.8, 4.9))
    ax.bar(x - width / 2.0, success, width, label="success", color="#1B9E77")
    ax.bar(x + width / 2.0, near_miss, width, label="near-miss", color="#D95F02")
    ax.set_xticks(x)
    ax.set_xticklabels(comparison_names, rotation=20, ha="right")
    ax.set_ylabel("count over 270 regimes")
    ax.set_title("Soft hybrid comparison")
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(BASELINE_PNG, dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8.0, 5.1))
    image = format_success_map(ax, success_grid(hybrid_rows, best_hybrid_name), f"{best_hybrid_name} success map (thrust=10000, target scale=1.00)")
    fig.colorbar(image, ax=ax, ticks=[0, 1], fraction=0.045, pad=0.03)
    fig.tight_layout()
    fig.savefig(SUCCESS_MAP_PNG, dpi=220)
    plt.close(fig)


def write_summary(hybrid_rows: Sequence[Dict[str, object]], ranking: Sequence[Dict[str, object]], reference_rows: Sequence[Dict[str, object]]) -> None:
    refs = {row["variant_name"]: row for row in aggregate_rows(reference_rows, ["adaptive_soft", "prewindow_radial_medium", "hard_hybrid_1e4"])}
    adaptive = refs.get("adaptive_soft")
    prewindow = refs.get("prewindow_radial_medium")
    hard = refs.get("hard_hybrid_1e4")
    best = ranking[0]
    success_delta_pre = int(best["success_count"]) - int(prewindow["success_count"]) if prewindow else 0
    near_delta_pre = int(best["near_miss_count"]) - int(prewindow["near_miss_count"]) if prewindow else 0
    near_delta_adaptive = int(best["near_miss_count"]) - int(adaptive["near_miss_count"]) if adaptive else 0
    success_delta_hard = int(best["success_count"]) - int(hard["success_count"]) if hard else 0
    near_delta_hard = int(best["near_miss_count"]) - int(hard["near_miss_count"]) if hard else 0
    maintains_high = prewindow is not None and int(best["success_count"]) >= int(prewindow["success_count"]) - 2
    reduces_near = prewindow is not None and int(best["near_miss_count"]) < int(prewindow["near_miss_count"])
    beats_hard = hard is not None and (
        int(best["success_count"]) > int(hard["success_count"])
        or (
            int(best["success_count"]) == int(hard["success_count"])
            and int(best["near_miss_count"]) < int(hard["near_miss_count"])
        )
    )

    best_cases = {case_key(row) for row in hybrid_rows if row["variant_name"] == best["variant_name"] and bool(row["success"])}
    pre_cases = {case_key(row) for row in reference_rows if row["variant_name"] == "prewindow_radial_medium" and bool(row["success"])}
    adaptive_cases = {case_key(row) for row in reference_rows if row["variant_name"] == "adaptive_soft" and bool(row["success"])}

    if maintains_high and reduces_near:
        tradeoff_text = "the soft hybrid combines the desired strengths on this grid"
    elif maintains_high:
        tradeoff_text = "the soft hybrid preserves reachability but still trades off near-miss stability"
    elif reduces_near:
        tradeoff_text = "the soft hybrid improves near-miss stability but gives up reachability"
    else:
        tradeoff_text = "the soft hybrid does not beat the Phase 7 reachability/stability trade-off"
    ...
    for row in ranking:
        lines.append(
            f"- `{row['variant_name']}` schedule `{row['alpha_schedule']}` mid `{float(row['alpha_mid']):.1e}` width `{float(row['alpha_width']):.1e}`: success `{row['success_count']}`, CAPTURE `{row['capture_count']}`, near-miss `{row['near_miss_count']}`."
        )
    lines.extend(
        [
            "",
    ...
    if adaptive:
        lines.append(f"- `adaptive_soft`: success `{adaptive['success_count']}`, CAPTURE `{adaptive['capture_count']}`, near-miss `{adaptive['near_miss_count']}`.")
    if prewindow:
        lines.append(f"- `prewindow_radial_medium`: success `{prewindow['success_count']}`, CAPTURE `{prewindow['capture_count']}`, near-miss `{prewindow['near_miss_count']}`.")
    if hard:
        lines.append(f"- `hard_hybrid_1e4`: success `{hard['success_count']}`, CAPTURE `{hard['capture_count']}`, near-miss `{hard['near_miss_count']}`.")
    lines.extend(
        [
            f"- Best soft hybrid: `{best['variant_name']}` with success `{best['success_count']}`, CAPTURE `{best['capture_count']}`, near-miss `{best['near_miss_count']}`.",
            "",
            "## Answers",
            "",
            f"1. Does soft hybrid recover the high reachability of prewindow_radial_medium? `{'yes' if maintains_high else 'no'}`. Delta vs prewindow_radial_medium is success `{success_delta_pre:+d}`.",
            f"2. Does it reduce near-miss compared with prewindow_radial_medium? `{'yes' if reduces_near else 'no'}`. Delta vs prewindow_radial_medium is near-miss `{near_delta_pre:+d}`; delta vs adaptive_soft is `{near_delta_adaptive:+d}`.",
            f"3. Does it beat hard hybrid? `{'yes' if beats_hard else 'no'}`. Delta vs hard_hybrid_1e4 is success `{success_delta_hard:+d}`, near-miss `{near_delta_hard:+d}`.",
            f"4. Which alpha schedule works best? `{best['variant_name']}` (`{best['alpha_schedule']}`, mid `{float(best['alpha_mid']):.1e}`, width `{float(best['alpha_width']):.1e}`).",
            f"5. Final project conclusion: orbit insertion control on this 2D grid is structurally sensitive to approach geometry before window entry. Pre-window radial shaping creates the largest reachability gain; adaptive WS improves local window behavior; hard boundaries are brittle; continuous action coordination is the right structure to test when combining explicit phases, but the final choice remains empirical because reachability and near-miss stability can still trade off. Current best trade-off assessment: `{tradeoff_text}`.",
        ]
    )
    SUMMARY_PATH.write_text("\n".join(lines), encoding="utf-8")


def iter_cases(variants: Sequence[SoftHybridVariant]) -> Iterable[tuple[SoftHybridVariant, float, float, float, float]]:
...
def main() -> None:
    ...
    if pending:
        ...
        print(f"phase76 pending={len(pending)} completed={len(done)} total={total} workers={workers}")
        if workers == 1:
            for idx, (variant, r0, angle, thrust, target_scale) in pending:
                ...
                print(
                    f"phase76 {idx}/{total} variant={variant.name} r0={r0:.5f} angle={angle:g} "
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                ...
                for future in as_completed(future_to_case):
                    ...
                    completed_now += 1
                    print(
                        f"phase76 {idx}/{total} done_now={completed_now}/{len(pending)} variant={variant.name} "
        ...
        print(f"phase76 no pending rows; completed={len(done)} total={total}")

    rows = load_rows()
    ranking = rank_rows(rows, variants)
    reference_rows = load_reference_rows()
    write_ranking(ranking)
    save_plots(rows, ranking, reference_rows)
    write_summary(rows, ranking, reference_rows)
    print(f"Saved Phase 7.6 soft hybrid outputs to {OUTPUT_DIR}")
