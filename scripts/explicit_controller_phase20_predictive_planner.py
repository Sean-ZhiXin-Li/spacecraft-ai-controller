...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase20_predictive_planner"
...
COMPARISON_CSV = OUTPUT_DIR / "reduced_grid_results.csv"
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase20_predictive_planner", "predictive"),
    ]
...
def rollout_case(
    ...
    def predictive_candidates(
        ...
        return [(name, *normalize_action(ax, ay, max_norm=0.16)) for name, ax, ay in raw]
    ...
    def simulate_predictive_candidate(
        ...
        crossed = False
        crossing_step = PREDICTIVE_HORIZON + 1
        ...
        for sim_step in range(1, PREDICTIVE_HORIZON + 1):
    ...
    def predictive_planner_action(
        ...
        if plan_steps_remaining <= 0:
            ...
            plan_steps_remaining = REPLAN_INTERVAL
            if best_name == "coast":
                planned_action_x, planned_action_y = 0.0, 0.0

        plan_steps_remaining -= 1
        mode = "coast" if abs(planned_action_x) + abs(planned_action_y) <= 1.0e-12 else "predictive_descent"
        return (
            planned_action_x,
            planned_action_y,
            mode,
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase20 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
