...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase22_two_burn_transfer"
...
RESULTS_CSV = OUTPUT_DIR / "phase22_results.csv"
...
def rollout_phase22_case(
    ...
    row: Dict[str, object] = {
        "controller_name": "phase22_two_burn_transfer",
        "r0_over_target": r0,
...
def run_phase22_rows() -> List[Dict[str, object]]:
    controller = ControllerSpec("phase22_two_burn_transfer")
    rows: List[Dict[str, object]] = []
    cases = list(iter_cases([controller]))
    for idx, (_controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        row = rollout_phase22_case(r0, angle, thrust, target_scale)
        rows.append(row)
        print(
            f"phase22 {idx}/{len(cases)} r0={r0:g} angle={angle:g} thrust={thrust:g} "
...
def phase22_stage_counts(rows: Sequence[Dict[str, object]]) -> Counter[str]:
    counts: Counter[str] = Counter()
    for row in rows:
        if str(row.get("controller_name")) != "phase22_two_burn_transfer":
            continue
...
def save_stage_usage_plot(rows: Sequence[Dict[str, object]]) -> None:
    counts = phase22_stage_counts(rows)
    ...
    ax.set_title("Phase 22 transfer stage usage")
...
def save_insertion_window_plot(stats: Dict[str, Dict[str, object]]) -> None:
    ...
    values = [
        phase22["insertion_attempts"],
        phase22["insertion_windows"],
        phase22["successful_insertion_windows"],
        phase22["handoff_entered"],
        phase22["capture"],
        phase22["success"],
    ]
    fig, ax = plt.subplots(figsize=(8.8, 5.0))
    ax.bar(labels, values, color=["#4C78A8", "#72B7B2", "#54A24B", "#F58518", "#E45756", "#B279A2"])
    ax.set_title("Phase 22 insertion-window outcomes")
    ax.set_ylabel("count")
...
def save_diagnostic_plots() -> None:
    ...
    for old_plot in TRAJECTORY_DIR.glob("phase22_representative_*.png"):
        old_plot.unlink()
    trace_rows = [rollout_phase22_case(r0, angle, thrust, TARGET_RADIUS_SCALE, record_trajectory=True) for r0, angle, thrust, _label in representative_cases()]
    ...
    for idx, row in enumerate(trace_rows, start=1):
        ...
        fig.suptitle(f"Phase 22 representative {outcome} transfer {idx}: {label}")
        fig.tight_layout()
        fig.savefig(TRAJECTORY_DIR / f"phase22_representative_{outcome}_{idx:02d}.png", dpi=220)
        plt.close(fig)

    for ax, title, ylabel in [
        (ax_e, "Phase 22 energy error vs time", "|E - E_target| / |E_target|"),
        (ax_h, "Phase 22 angular momentum error vs time", "|h - h_target| / |h_target|"),
    ]:
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, object]]) -> None:
    ...
    phase22 = stats["phase22_two_burn_transfer"]
    crossing_improved = int(phase22["crossing_cases"]) > int(baseline["crossing_cases"])
    recoverable_improved = int(phase22["recoverable_crossings"]) > int(baseline["recoverable_crossings"])
    capture_improved = int(phase22["capture"]) > int(baseline["capture"])
    success_improved = int(phase22["success"]) > int(baseline["success"])
    bottleneck = "transfer timing"
    if int(phase22["insertion_windows"]) > 0 and int(phase22["recoverable_crossings"]) == 0:
        bottleneck = "insertion geometry"
    if int(phase22["recoverable_crossings"]) > 0 and not capture_improved:
        bottleneck = "CAPTURE architecture"
    if int(phase22["insertion_windows"]) == 0 and not crossing_improved:
        bottleneck = "transfer timing or physical reachability"
    mode_counts = phase22_stage_counts(rows)
    lines = [
        ...
        "|---|---:|---:|---:|---:|---:|",
        f"| Phase 7.6 `baseline_soft_linear_3e4` | {baseline['crossing_cases']} | {baseline['recoverable_crossings']} | {baseline['capture']} | {baseline['success']} | {baseline['overspeed']} |",
        f"| Phase 20 `phase20_predictive_planner` | {phase20['crossing_cases']} | {phase20['recoverable_crossings']} | {phase20['capture']} | {phase20['success']} | {phase20['overspeed']} |",
        f"| Phase 21 `phase21_orbital_transfer_planner` | {phase21['crossing_cases']} | {phase21['recoverable_crossings']} | {phase21['capture']} | {phase21['success']} | {phase21['overspeed']} |",
        f"| Phase 22 `phase22_two_burn_transfer` | {phase22['crossing_cases']} | {phase22['recoverable_crossings']} | {phase22['capture']} | {phase22['success']} | {phase22['overspeed']} |",
        "",
        "## Phase 22 Transfer Diagnostics",
        "",
        f"- Burn A steps: `{phase22['burn_a_steps']}`.",
        f"- Burn B steps: `{phase22['burn_b_steps']}`.",
        f"- Insertion attempts: `{phase22['insertion_attempts']}`.",
        f"- Insertion windows: `{phase22['insertion_windows']}`.",
        f"- Successful insertion windows: `{phase22['successful_insertion_windows']}`.",
        f"- Handoff-entered cases: `{phase22['handoff_entered']}`.",
        f"- Stage usage: `{json.dumps(dict(mode_counts), sort_keys=True)}`.",
        f"- Mean final energy error: `{phase22['mean_energy_error']:.4f}`.",
        f"- Mean final angular momentum error: `{phase22['mean_angular_momentum_error']:.4f}`.",
        "",
        "## Research Answers",
        "",
        f"1. Can Burn A create new orbital corridors? `{'yes' if int(phase22['insertion_windows']) > 0 else 'no'}`. Insertion windows: `{phase22['insertion_windows']}`.",
        f"2. Does coast arc improve geometry more than local planning? `{'yes' if crossing_improved else 'no'}` by crossing-count criterion.",
        f"3. Can Burn B create recoverable crossings? `{'yes' if int(phase22['recoverable_crossings']) > 0 else 'no'}`. Recoverable crossings: `{phase22['recoverable_crossings']}`.",
        f"4. Does Phase 22 finally exceed Phase 7.6? `{'yes' if (crossing_improved or capture_improved or success_improved) else 'no'}`.",
        f"5. If not, current bottleneck: `{bottleneck}`.",
        "",
        "## Success Criteria",
        "",
        f"- Minimum, crossings > Phase 7.6: `{'met' if crossing_improved else 'not met'}`.",
        f"- Moderate, recoverable crossings > 0: `{'met' if int(phase22['recoverable_crossings']) > 0 else 'not met'}`.",
        ...
        f"- Research success, new insertion windows or recoverable crossings: `{'met' if (int(phase22['insertion_windows']) > 0 or int(phase22['recoverable_crossings']) > 0) else 'not met'}`.",
        "",
        "## Honesty Note",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 22 two-burn transfer benchmark.")
    ...
    phase22_rows = run_phase22_rows()
    rows = reference_rows + phase22_rows
    write_results(rows)
    ...
    print(f"Saved Phase 22 two-burn transfer outputs to {OUTPUT_DIR}")
