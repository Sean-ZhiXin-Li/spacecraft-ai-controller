...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase15_oscillation"
...
SUMMARY_MD = OUTPUT_DIR / "phase15_summary.md"
TRAJECTORY_DIR = OUTPUT_DIR / "trajectories"
RADIUS_OSCILLATION_PNG = OUTPUT_DIR / "radius_oscillation_examples.png"
RADIAL_VELOCITY_PNG = OUTPUT_DIR / "radial_velocity_examples.png"
...
PLANNER_MODES = ["burn", "coast", "event_burn", "oscillation", "soft_window"]
MAX_BURN_STEPS = 30
COOLDOWN_STEPS = 20
EVENT_BURN_STEPS = 10
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase14_event_guidance", "event"),
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase15_oscillation"]
...
def save_crossing_count_plot(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]]) -> None:
    labels = ["baseline", "phase14", "phase15"]
    names = ["baseline_soft_linear_3e4", "phase14_event_guidance", "phase15_oscillation"]
    ...
    ax.set_title("Phase 15 crossing and capture comparison")
...
def save_trajectory_plot(controller: ControllerSpec, r0: float, angle: float, thrust: float, idx: int) -> None:
    ...
    fig.suptitle(
        f"Phase 15 oscillation r0={r0:g}, angle={angle:g}, thrust={thrust:g}, "
...
def save_trajectory_diagnostics() -> None:
    ...
    controller = ControllerSpec("phase15_oscillation", "oscillation")
    cases = [
        (0.98, 150.0, 8000.0),
        (0.98, 175.0, 12000.0),
        (1.00, 170.0, 10000.0),
...
def save_radius_oscillation_plot() -> None:
    ...
    ax.set_title("Phase 15 radius oscillation examples")
...
def save_radial_velocity_plot() -> None:
    ...
    ax.set_title("Phase 15 radial velocity examples")
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=8)
    fig.tight_layout()
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    ...
    phase14 = stats["phase14_event_guidance"]
    planner = stats["phase15_oscillation"]
    capture_improvement = planner["capture"] - baseline["capture"]
    success_improvement = planner["success"] - baseline["success"]
    crossing_improvement = sum(bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "phase15_oscillation") - sum(
        bool(row["crossing_occurs"]) for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"
    )
    phase14_rows = [row for row in rows if str(row["controller_name"]) == "phase14_event_guidance"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase15_oscillation"]
    no_capture_baseline = baseline["total"] - baseline["capture"]
    no_capture_planner = planner["total"] - planner["capture"]
    overspeed_phase14 = sum(bool(row.get("overspeed", False)) for row in phase14_rows)
    overspeed_planner = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    overspeed_reduction = overspeed_phase14 - overspeed_planner
    ...
    lines = [
        ...
        f"| `phase14_event_guidance` | {phase14['capture']} | {phase14['success']} | {phase14['near_miss']} | {phase14['total']} |",
        f"| `phase15_oscillation` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        f"- Total radius crossings: baseline `{baseline_crossings}`, Phase 15 `{total_crossings}`.",
        f"- Turning points detected in Phase 15: `{total_turning_points}`.",
        f"- No-CAPTURE cases: baseline `{no_capture_baseline}`, planner `{no_capture_planner}`.",
        f"- Overspeed terminations: Phase 14 `{overspeed_phase14}`, Phase 15 `{overspeed_planner}`.",
        f"- Overspeed reduction vs Phase 14: `{overspeed_reduction}`.",
        ...
        "## Answers",
        "",
        f"1. Does oscillation control create new crossings? `{'yes' if crossing_improvement > 0 or total_crossings > baseline_crossings else 'no'}`. Total crossings change from `{baseline_crossings}` to `{total_crossings}`.",
        f"2. Does capture increase? It `{honest_result}` CAPTURE: CAPTURE changes by `{capture_improvement}` versus baseline.",
        f"3. Does overspeed remain controlled? `{'yes' if overspeed_planner <= overspeed_phase14 else 'no'}`. Phase 15 overspeed count is `{overspeed_planner}`.",
        f"4. Does this produce usable events for future event guidance? `{'partially' if oscillation_created else 'no'}`. Turning points observed: `{total_turning_points}`.",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase15 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
    ...
    print(f"Saved Phase 15 oscillation outputs to {OUTPUT_DIR}")
