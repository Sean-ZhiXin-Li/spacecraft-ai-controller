...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "phase19_planner"
...
SUMMARY_MD = OUTPUT_DIR / "phase19_summary.md"
...
def define_controllers() -> List[ControllerSpec]:
    return [
        ...
        ControllerSpec("phase19_minimal_transfer", "minimal_transfer"),
    ]
...
def write_planner_diagnostics(rows: Sequence[Dict[str, object]]) -> Dict[str, object]:
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase19_minimal_transfer"]
...
def write_summary(rows: Sequence[Dict[str, object]], stats: Dict[str, Dict[str, int]], planner_diagnostics: Dict[str, object]) -> None:
    ...
    phase18 = stats["phase18_crossing_targeting"]
    planner = stats["phase19_minimal_transfer"]
    baseline_rows = [row for row in rows if str(row["controller_name"]) == "baseline_soft_linear_3e4"]
    phase18_rows = [row for row in rows if str(row["controller_name"]) == "phase18_crossing_targeting"]
    planner_rows = [row for row in rows if str(row["controller_name"]) == "phase19_minimal_transfer"]
    baseline_crossings = sum(int(float(row["radius_crossings_total"])) for row in baseline_rows)
    phase18_crossings = sum(int(float(row["radius_crossings_total"])) for row in phase18_rows)
    ...
    phase18_crossing_cases = sum(bool(row["crossing_occurs"]) for row in phase18_rows)
    ...
    capture_improvement_vs_phase18 = planner["capture"] - phase18["capture"]
    success_improvement_vs_phase18 = planner["success"] - phase18["success"]
    crossing_case_improvement_vs_phase18 = planner_crossing_cases - phase18_crossing_cases
    baseline_overspeed = sum(str(row["termination_reason"]) == "overspeed" for row in baseline_rows)
    phase18_overspeed = sum(str(row["termination_reason"]) == "overspeed" for row in phase18_rows)
    planner_overspeed = sum(str(row["termination_reason"]) == "overspeed" for row in planner_rows)
    injection_steps = sum(int(float(row["burn_steps"])) for row in planner_rows)
    coast_steps = sum(int(float(row["coast_steps"])) for row in planner_rows)
    targeting_steps = 0
    for row in planner_rows:
        counts = json.loads(str(row.get("planner_mode_counts_json", "{}")))
        targeting_steps += int(counts.get("targeting", 0)) + int(counts.get("targeting_close", 0))
    creates_crossings = planner_crossing_cases > phase18_crossing_cases
    improves_capture = capture_improvement_vs_phase18 > 0
    coast_dominates = coast_steps > injection_steps and coast_steps > targeting_steps
    coast_result = "yes" if coast_dominates and planner_crossing_cases > 0 and targeting_steps > 0 else "inconclusive"
    outperforms_reactive = capture_improvement_vs_phase18 > 0 or success_improvement_vs_phase18 > 0
    lines = [
        ...
        f"- The injection burn is small and purely tangential: norm `{INJECTION_NORM:.2f}`, with duration capped between {INJECTION_MIN_STEPS} and {INJECTION_MAX_STEPS} steps.",
        f"- After injection, the planner forces at least `{FORCED_COAST_STEPS}` no-thrust coast steps before targeting can engage.",
        "",
        "## Reduced-Grid Result",
        "",
        ...
        f"| `phase18_crossing_targeting` | {phase18['capture']} | {phase18['success']} | {phase18['near_miss']} | {phase18['total']} |",
        f"| `phase19_minimal_transfer` | {planner['capture']} | {planner['success']} | {planner['near_miss']} | {planner['total']} |",
        "",
        ...
        f"- Capture improvement vs Phase 18 reactive targeting: `{capture_improvement_vs_phase18}`.",
        f"- Success improvement vs Phase 18 reactive targeting: `{success_improvement_vs_phase18}`.",
        f"- Crossing-case improvement vs Phase 18 reactive targeting: `{crossing_case_improvement_vs_phase18}`.",
        f"- Total radius crossings: baseline `{baseline_crossings}`, Phase 18 `{phase18_crossings}`, Phase 19 `{planner_crossings}`.",
        f"- Overspeed terminations: baseline `{baseline_overspeed}`, Phase 18 `{phase18_overspeed}`, Phase 19 `{planner_overspeed}`.",
        f"- Phase 19 injection steps: `{injection_steps}`.",
        f"- Phase 19 coast steps: `{coast_steps}`.",
        f"- Phase 19 targeting steps: `{targeting_steps}`.",
        f"- Phase 19 mode usage: `{json.dumps(planner_diagnostics.get('descent_step_counts', {}), sort_keys=True)}`.",
        "",
        "## Answers",
        "",
        f"1. Does planned injection create new crossings? `{'yes' if creates_crossings else 'no'}`. Crossing cases change from Phase 18 `{phase18_crossing_cases}` to Phase 19 `{planner_crossing_cases}`.",
        f"2. Does capture increase? `{'yes' if improves_capture else 'no'}`. CAPTURE changes by `{capture_improvement_vs_phase18}` versus Phase 18.",
        f"3. Is coast phase critical? `{coast_result}`. Coast dominates the mode counts (`{coast_steps}` coast steps vs `{injection_steps}` injection and `{targeting_steps}` targeting), but this run produced `{planner_crossing_cases}` Phase 19 crossing cases and does not include a separate no-coast ablation.",
        f"4. Does planning outperform reactive control? `{'yes' if outperforms_reactive else 'no'}` on this reduced grid. The minimal injection/coast plan only outperforms Phase 18 if CAPTURE or success increases.",
        "",
        "## Interpretation",
        "",
...
def main() -> None:
    ...
    for idx, (controller, r0, angle, thrust, target_scale) in enumerate(cases, start=1):
        ...
        rows.append(row)
        print(
            f"phase19 {idx}/{len(cases)} controller={controller.name} r0={r0:g} angle={angle:g} "
    ...
    print(f"Saved Phase 19 minimal-transfer outputs to {OUTPUT_DIR}")
