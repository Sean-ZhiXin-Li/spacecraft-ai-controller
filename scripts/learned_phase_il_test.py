...
OUTPUT_DIR = PROJECT_ROOT / "analysis" / "learned_phase_il"
MODEL_DIR = PROJECT_ROOT / "models"
MODEL_PATH = MODEL_DIR / "learned_phase_il_policy.pth"
SUMMARY_PATH = OUTPUT_DIR / "learned_phase_il_summary.json"
RADIUS_PLOT_PATH = OUTPUT_DIR / "learned_phase_il_radius.png"
VR_PLOT_PATH = OUTPUT_DIR / "learned_phase_il_v_r.png"
PHASE_PLOT_PATH = OUTPUT_DIR / "learned_phase_il_phase.png"
NOTE_PATH = PROJECT_ROOT / "analysis" / "learned_phase_il_result.md"
HISTORY_SUMMARY_PATH = PROJECT_ROOT / "analysis" / "history_aware_il" / "history_aware_il_summary.json"
...
def train_model(
    ...
    actions: np.ndarray,
    labels: np.ndarray,
) -> tuple[LearnedPhasePolicy, Dict[str, float]]:
    ...
    model = LearnedPhasePolicy(input_dim=int(history_inputs.shape[1])).to(DEVICE)
    ...
    for _ in range(epochs):
        ...
        for x_batch, action_batch, phase_batch in loader:
            ...
            action_pred, phase_logits = model(x_batch)
...
def load_history_reference() -> Dict[str, object] | None:
    if not HISTORY_SUMMARY_PATH.exists():
        return None
    return json.loads(HISTORY_SUMMARY_PATH.read_text(encoding="utf-8"))


def improves_over_history(eval_metrics: Dict[str, object], history_summary: Dict[str, object] | None) -> bool | None:
    if history_summary is None:
        return None
    history_eval = history_summary.get("eval", {})
    if bool(eval_metrics["success"]) != bool(history_eval.get("success", False)):
        return bool(eval_metrics["success"])
    if bool(eval_metrics["crossing_occurs"]) != bool(history_eval.get("crossing_occurs", False)):
        return bool(eval_metrics["crossing_occurs"])
    return float(eval_metrics["final_radius_error"]) < float(history_eval.get("final_radius_error", float("inf")))
...
def save_phase_plot(path: Path, trace: Dict[str, List[float] | List[int]]) -> None:
    ...
    plt.title("Learned-phase IL | predicted phase vs time")
    plt.tight_layout()
    plt.savefig(path, dpi=180)
    plt.close()
...
def write_note(summary: Dict[str, object]) -> None:
    ...
    history_eval = summary.get("history_aware_reference_eval")
    crossing = bool(eval_metrics["crossing_occurs"])
    success = bool(eval_metrics["success"])
    improved = summary.get("improves_over_history_aware")
    if improved is None:
        improved_text = "Unknown; no saved history-aware summary was available for comparison."
    else:
        improved_text = "Yes." if improved else "No."

    missing = "Nothing in this test achieved full success." if success else (
        ...
        if crossing
        else "The learned internal phase signal still does not trigger a robust autonomous descent-to-capture transition."
    )
    ...
    lines = [
        ...
        "## Main Answer",
        "",
        f"- Does learned phase representation recover crossing without oracle phase input? `{'Yes' if crossing else 'No'}`",
        f"- Does it improve over history-aware imitation? `{improved_text}`",
    ...
    if isinstance(history_eval, dict):
        lines.extend(
            [
                "",
                "## History-Aware Reference",
                "",
                f"- crossing_occurs `{history_eval.get('crossing_occurs')}`",
                f"- radius_crossings_total `{history_eval.get('radius_crossings_total')}`",
                f"- first_crossing_step `{history_eval.get('first_crossing_step')}`",
                f"- success `{history_eval.get('success')}`",
                f"- final_radius_error `{float(history_eval.get('final_radius_error', 0.0)):.3e}`",
            ]
        )
...
def main() -> None:
    ...
    history_summary = load_history_reference()
    history_eval = history_summary.get("eval") if history_summary else None

    env = make_env()
    save_plot(RADIUS_PLOT_PATH, trace, "radius", "Learned-phase IL | radius vs time", "radius [m]", target_value=env.target_radius)
    save_plot(VR_PLOT_PATH, trace, "v_r", "Learned-phase IL | v_r vs time", "v_r [m/s]", target_value=0.0)
    ...
    summary = {
        ...
        "phase_labels": PHASES,
        "phase_input_source": "learned_from_history_only",
        "seed": SEED,
        "device": str(DEVICE),
        "num_samples": int(len(states)),
        "train": train_metrics,
        "eval": eval_metrics,
        "history_aware_reference_eval": history_eval,
        "improves_over_history_aware": improves_over_history(eval_metrics, history_summary),
        "predicted_phase_counts": phase_counts(predicted_phase),
        "predicted_phase_transitions": phase_transition_summary(predicted_phase),
