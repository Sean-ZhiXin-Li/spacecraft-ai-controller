...
5. Current Phase 7.5 conclusion: within the 2D Python evaluator, explicit controllers can beat the learned baseline on this local orbit-lock grid; Phase 7's pre-window radial shaping is the main reachability gain, while Phase 7.5 tests whether a threshold switch can recover stability without changing physics or learning. The current controller choice should prioritize the ranked grid result and near-miss tolerance rather than adding new algorithmic scope.
